...
def assert_pto_isa_root_matches_pin(pto_isa_root: str) -> None:
    """Raise when ``pto_isa_root`` is not the checkout for the current pin.

    Kernel / host compile paths must use the managed revision from
    ``pto_isa.pin``. Callers may still pass an explicit path, but it cannot
    drift off-pin (#1403 pathway hygiene).
    """
    ...
    if actual_commit == required_commit:
        return
    raise RuntimeError(
        "PTO-ISA checkout does not match pto_isa.pin: "
        f"required {required_commit}, but {pto_isa_root} is at "
        f"{actual_commit or '(unknown)'}.\n"
        "Use ensure_pto_isa_root() (or rebuild the managed checkout under "
        "build/pto-isa) so compile uses the pinned revision."
    )
