...
# -----------------------------------------------------------------------------------------------------------
"""Golden reference + fixture for the Qwen3-14B 3-layer decode SceneTestCase.
...
the paged KV pool are STACKED along dim 0 (one slice per layer); the three layers
...
def generate_inputs(seed: int = 1234, seq_len: int = DEFAULT_SEQ_LEN) -> TaskArgsBuilder:
    """Deterministic fixture for decode_fwd_layers (N=3), stacked x3 along dim 0.

    Every lane uses sequence length ``seq_len`` (default 3500, the stress prompt).
    Per-layer weights are replicated (stack0) so layers 1/2 reuse layer 0's weights,
...
def compute_golden(args: TaskArgsBuilder) -> None:
    """Fill ``args.out`` (and INOUT k_cache/v_cache) for the 3-layer decode chunk."""
