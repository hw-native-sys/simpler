...
"""Critical-path analysis over an chip-swimlane build_output.

Given a run directory produced by a ``--enable-chip-swimlane`` run, this
...
Inputs consumed per rank (auto-discovered under the given directory):
  - ``chip_swimlane_records.json`` : per (task, core-block) execution slices
...
# Structured per-task L2 records file. Current runs emit ``chip_swimlane_records.json``;
# ``l2_perf_records.json`` is the name used in some docs / older runtimes. The first
# name that exists in a rank dir is used.
SWIMLANE_RECORD_NAMES = ("chip_swimlane_records.json", "l2_perf_records.json")
...
def build_graph(rank_dir: Path, root: Path, tol: int) -> RankGraph:
    ...
    # receive_to_start_cycles] (see ChipSwimlaneCollector::export_swimlane_json). Kernel
...
def render_report(results: list[RankResult], top: int) -> str:
    out: list[str] = []
    w = out.append
    w("# Critical-path report (chip swimlane)\n")
    w(
...
def _critical_trace(trace_data, critical_task_ids: set[str]):
    ...
    for event in events:
        ...
        if event.get("ph") != "X" or event.get("pid") != 4 or task_id is None or str(task_id) in critical_task_ids:
            continue
        event["name"] = f"·({task_display(task_id)})"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Reconstruct the critical path of an chip-swimlane run tree "
    ...
    p.add_argument(
        "build_dir",
        type=Path,
        help="directory containing outputs/... or dfx_outputs/.../chip_swimlane_records.json artifacts",
    )
    p.add_argument(
        "--report",
...
def main(argv: list[str]) -> int:
    ...
    if not rank_dirs:
        print(
            f"error: no chip_swimlane_records.json (+deps.json+name_map*.json) found "
            f"under {root}. Did the run use --enable-chip-swimlane?",
            file=sys.stderr,
        )
        return 2
