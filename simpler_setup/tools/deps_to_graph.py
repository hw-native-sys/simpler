...
In text mode the default view shows task identity ``(ring, local)`` with a
``func=`` label (resolved from perf sidecar + name map, or from ``kernel_ids``
fallback when no sidecar is present). In HTML mode nodes are labeled
``(ring, local)`` only; they are colored by ``core_type`` when a perf sidecar
is colocated (AIC blue, AIV orange).

Usage:
    python -m simpler_setup.tools.deps_to_graph DEPS_JSON
    python -m simpler_setup.tools.deps_to_graph DEPS_JSON --format text
    python -m simpler_setup.tools.deps_to_graph DEPS_JSON --format html --engine sfdp
...
def _task_func_id_from_kernel_ids(task_entry):
    ...
    for kernel_id in kernel_ids:
        func_id = _normalize_small_int(kernel_id)
        if func_id is not None and func_id >= 0:
            return func_id
    return None
...
def _merge_task_meta_with_kernel_ids(meta, task_table, func_names=None):
    ...
    for task_id, task_entry in task_table.items():
        func_id = _task_func_id_from_kernel_ids(task_entry)
        if func_id is None:
            continue
        entry = merged.setdefault(task_id, {})
        if entry.get("func_id") is None:
            entry["func_id"] = func_id
        if not entry.get("func_name") and func_names:
            stored_id = entry.get("func_id")
            entry["func_name"] = func_names.get(str(stored_id)) or func_names.get(stored_id)
    return merged
...
def _task_func_name(task_id, meta):
...
def emit_text(edges, nodes, meta, deps_path, annotations=None, tensor_table=None, task_table=None):
    ...
    Output shape:
        SUMMARY
        TASK INDEX
        per-task detail blocks with PREDS / SUCCS / optional ARGS sections
    """
    ...
    for task_id in sorted_nodes:
        func_name = _task_func_name(task_id, meta)
        lines.append(
            f"TASK {fmt_task(task_id)} func={func_name} "
    ...
    for task_id in sorted_nodes:
        func_name = _task_func_name(task_id, meta)
        lines.append("")
        lines.append(f"=== TASK {fmt_task(task_id)} func={func_name} ===")
...
Examples:
  ...
  %(prog)s deps.json --format html
""",
    )
    p.add_argument("input", nargs="?", help="Path to deps.json (default: newest under ./outputs/).")
    p.add_argument("-o", "--output", help="Output path (default: deps_graph.txt for text, deps_graph.html for html).")
    p.add_argument(
        "--format",
        choices=["text", "html"],
        default="text",
...
def main(argv=None):
    ...
    out = (
        Path(args.output)
        if args.output
        else input_path.parent / ("deps_graph.txt" if args.format == "text" else "deps_graph.html")
    )
    if args.format == "text":
        text = emit_text(
            edges,
