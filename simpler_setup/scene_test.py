...
class SceneTestCase:
    ...
    def _compile_l3_callables(cls, platform):
        ...
        compiled = {}
        pin = _pto_isa_compile_cache_token()
        for entry in cls.CALLABLE["callables"]:
            if "orchestration" in entry:
                name = entry["name"]
                cache_key = (cls.__qualname__, name, platform, cls._st_runtime, pin)
    ...
    for cls in group:
        ...
        for entry in cls.CALLABLE.get("callables", []):
            if "callable" in entry:
                ...
                name = entry["name"]
                cache_key = (cls.__qualname__, name, args.platform, cls._st_runtime, pin)
