...
# simpler pin → managed checkout under the simpler worktree (no ambient
# PTO_ISA_ROOT for simpler itself — #1403). Set the path only if this
# external script still needs an explicit root; prefer the pin checkout:
export PTO_ISA_ROOT="$(cd ../.. && pwd)/build/pto-isa"
