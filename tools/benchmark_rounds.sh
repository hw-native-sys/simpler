...
Device-wall numbers are zero only when the runtime is built without
PTO2_PROFILING. The default build has it on, so the device column is
populated for every round regardless of --enable-l2-swimlane.
USAGE
            exit 0 ;;
        *) EXTRA_ARGS+=("$1"); shift ;;
    esac
