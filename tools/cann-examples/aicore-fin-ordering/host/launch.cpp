 ...
 * -----------------------------------------------------------------------------------------------------------
 */
// launch.cpp — host launcher for the fin-ordering example.
//
...
//                  path under test, and writes a FinResult block.
//
// Coordination is via a single GM-resident FinHandshake that both
...
void PrintResultTable(const FinResult &r, const std::vector<FinTrialRecord> &recs) {
    auto ns = [](uint64_t t) -> uint64_t {
        return t * 20;
    };  // 50 MHz counter
    static const char *kCName[] = {"C0 bare",       "C1 dsb sy+load", "C2 inv+load",  "C3 dsb+inv+load",
                                   "C4 delay(inv)", "C5 dmb ld+load", "C6 ldar flag", "C7 delay(dsb)"};

    std::printf("\n=== FIN-ordering result ===\n");
    std::printf("  magic                 = 0x%08x  %s\n", r.magic, r.magic == kFinResultMagic ? "OK" : "BAD");
    std::printf("  consumer_rc           = %d\n", r.consumer_rc);
    std::printf("  records_written       = %u\n", r.records_written);
    std::printf(
        "  payload_addr          = 0x%016lx  (%s 64B aligned)\n", (unsigned long)r.payload_addr,
        (r.payload_addr % 64 == 0) ? "is" : "NOT"
    );
    std::printf("  core_id raw / used    = 0x%08x / %u\n", r.core_id_raw, r.core_id_used);
    std::printf(
        "  producer publishes    = %lu  last_round = %lu  final_payload = %lu\n", (unsigned long)r.producer_publishes,
        (unsigned long)r.producer_last_round, (unsigned long)r.final_payload
    );
    std::printf("  cond_addr polled      = 0x%016lx\n", (unsigned long)r.cond_addr);
    std::printf(
        "  invalidate cost       = %lu ticks (~%lu ns)  -> C4 delay\n", (unsigned long)r.invalidate_cost_ticks,
        ns(r.invalidate_cost_ticks)
    );
    std::printf(
        "  dsb sy cost           = %lu ticks (~%lu ns)  -> C7 delay\n", (unsigned long)r.dsb_cost_ticks,
        ns(r.dsb_cost_ticks)
    );

    std::printf("\n  first-load outcome per cell. 'older' is the only value meaning\n");
    std::printf("  published data was not visible; 'newer' must not occur under the\n");
    std::printf("  frozen protocol and is reported as an anomaly, not a failure.\n\n");
    std::printf(
        "  %-4s %-4s %-18s %6s %6s %6s %6s %8s %8s %6s %8s\n", "P", "pre", "consumer", "trials", "equal", "older",
        "newer", "re-read", "post-inv", "unres", "to/o"
    );
    for (uint32_t pi = 0; pi < kFinProducerModeCount; pi++) {
        for (uint32_t pf = 0; pf < kFinPrefetchVariants; pf++) {
            for (uint32_t cm = 0; cm < kFinConsumerModeCount; cm++) {
                const FinCell &c = r.cells[fin_cell_index(pi, cm, pf)];
                if (c.trials == 0) continue;
                std::printf(
                    "  %-4s %-4s %-18s %6lu %6lu %6lu %6lu %8lu %8lu %6lu %8lu\n", pi == 0 ? "P0" : "P1",
                    pf == 1 ? "yes" : "no", kCName[cm], (unsigned long)c.trials, (unsigned long)c.first_equal,
                    (unsigned long)c.first_older, (unsigned long)c.first_newer, (unsigned long)c.reread_matched,
                    (unsigned long)c.invalidate_matched, (unsigned long)c.unresolved, (unsigned long)c.wait_timeouts
                );
            }
        }
    }
    std::printf("\n  P0 = store,dcci,FIN          P1 = store,dcci,dsb(DSB_DDR),FIN\n");
    std::printf("  pre = consumer deliberately read payload before the publish\n");
    std::printf("  re-read  = matched after ordinary re-reads, no cache maintenance\n");
    std::printf("  post-inv = matched only after the invalidate operation\n");
    std::printf("  unres    = never matched within the bound; reported, not attributed\n");

    uint64_t anomalies = 0, wait_to = 0, echo = 0;
    for (const auto &e : recs) {
        if (e.first_read == kFinFirstNewer) anomalies++;
        if (e.flags & kFinFlagWaitTimeout) wait_to++;
        if (e.flags & kFinFlagRoundEchoMismatch) echo++;
    }
    std::printf(
        "\n  anomalies: newer=%lu  wait-timeout=%lu  round-echo-mismatch=%lu\n", (unsigned long)anomalies,
        (unsigned long)wait_to, (unsigned long)echo
    );

    const char *dump = std::getenv("FIN_ORDER_RECORDS");
    if (dump != nullptr && !recs.empty()) {
        std::FILE *f = std::fopen(dump, "w");
        if (f != nullptr) {
            std::fprintf(
                f, "trial,producer_mode,consumer_mode,prefetched,raw_cond,expected_seq,first_payload,"
                   "first_read,reread_payload,reread_iters,post_inv_payload,ticks_to_match,flags\n"
            );
            for (const auto &e : recs) {
                std::fprintf(
                    f, "%u,%u,%u,%u,0x%08x,%lu,%lu,%u,%lu,%u,%lu,%lu,0x%x\n", e.trial_id, e.producer_mode,
                    e.consumer_mode, e.prefetched, e.raw_cond, (unsigned long)e.expected_seq,
                    (unsigned long)e.first_payload, e.first_read, (unsigned long)e.ordinary_reread_payload,
                    e.reread_iters, (unsigned long)e.post_invalidate_payload, (unsigned long)e.ticks_to_first_match,
                    e.flags
                );
            }
            std::fclose(f);
            std::printf("  per-trial records written to %s\n", dump);
        }
    }
}
...
int main(int argc, char **argv) {
    if (argc < 2) {
        ...
        std::fprintf(stderr, "  FIN_ORDER_CONSUMER_SO -- path to libfin_order_consumer.so\n");
        std::fprintf(stderr, "  FIN_ORDER_PRODUCER_O  -- path to fin_order_producer.o\n");
        return 1;
    }
    int device_id = std::atoi(argv[1]);
    uint32_t target_core_idx = (argc >= 3) ? static_cast<uint32_t>(std::atoi(argv[2])) : 0u;
    uint32_t rounds_per_cell = (argc >= 4) ? static_cast<uint32_t>(std::atoi(argv[3])) : 50u;
    uint32_t n_samples = rounds_per_cell * kFinProducerModeCount * kFinConsumerModeCount * kFinPrefetchVariants;
    ...
    const char *consumer_env = std::getenv("FIN_ORDER_CONSUMER_SO");
    std::string consumer_path = consumer_env ? consumer_env : "../device-aicpu/build/libfin_order_consumer.so";
    const char *producer_env = std::getenv("FIN_ORDER_PRODUCER_O");
    std::string producer_path = producer_env ? producer_env : "../device-aicore/build/fin_order_producer.o";
    ...
    std::printf(
        "[fin-order] aic_ctrl_base = 0x%016lx, target_core_idx = %u, n_samples = %u\n", aic_ctrl_base, target_core_idx,
        n_samples
    );

    // ---- Allocate GM ----
    DevBuf dev_dispatcher, dev_consumer, dev_args, dev_result, dev_handshake, dev_records;
    ...
    ACL_CHECK(dev_result.Alloc(sizeof(FinResult)), "Result GM");
    ACL_CHECK(dev_handshake.Alloc(sizeof(FinHandshake)), "Handshake GM");
    ACL_CHECK(aclrtMemset(dev_result.ptr, sizeof(FinResult), 0, sizeof(FinResult)), "memset result");
    ACL_CHECK(aclrtMemset(dev_handshake.ptr, sizeof(FinHandshake), 0, sizeof(FinHandshake)), "memset handshake");
    const uint32_t records_capacity = n_samples;
    const size_t records_bytes = sizeof(FinTrialRecord) * records_capacity;
    ACL_CHECK(dev_records.Alloc(records_bytes), "Records GM");
    ACL_CHECK(aclrtMemset(dev_records.ptr, records_bytes, 0, records_bytes), "memset records");
    ...
    {
        char tmpl[] = "/tmp/fin_order_consumer_XXXXXX.json";
    ...
    run_args.records_addr = reinterpret_cast<uint64_t>(dev_records.ptr);
    run_args.records_capacity = records_capacity;
    run_args.rounds_per_cell = rounds_per_cell;
    run_args.fallback_core_idx = target_core_idx;
    ACL_CHECK(
    ...
    FinResult result = {};
    ACL_CHECK(
        aclrtMemcpy(&result, sizeof(result), dev_result.ptr, sizeof(result), ACL_MEMCPY_DEVICE_TO_HOST), "D2H FinResult"
    );
    std::vector<FinTrialRecord> recs(result.records_written);
    if (result.records_written > 0) {
        ACL_CHECK(
            aclrtMemcpy(
                recs.data(), sizeof(FinTrialRecord) * recs.size(), dev_records.ptr,
                sizeof(FinTrialRecord) * recs.size(), ACL_MEMCPY_DEVICE_TO_HOST
            ),
            "D2H FinTrialRecord[]"
        );
    }
    PrintResultTable(result, recs);

    bool ok = (result.magic == kFinResultMagic) && (result.consumer_rc == 0) && (result.records_written > 0);
    return ok ? 0 : 2;
}
