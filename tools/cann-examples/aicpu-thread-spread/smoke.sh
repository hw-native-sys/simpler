...
export SIMPLER_AICPU_SPREAD_SO="$HERE/device/build/libaicpu_thread_spread.so"
# One launch count rather than the README's sweep: a smoke only has to prove the
# launch path works.
exec "$HERE/host/build/aicpu_thread_spread" "$DEV" 6
