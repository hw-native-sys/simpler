...
仿真把每个 AICore/AICPU“核”实现为一个 host 线程，默认由 OS 在全部物理核（本机 320 核 / 4 个 NUMA
节点）上自由调度。跨核迁移与跨 NUMA 访问会给 §6.2 的 `device` 墙钟带来明显抖动（单次运行间方差很
...
**最佳独占绑核策略。** 本机拓扑为 **8 个 NUMA 节点 × 40 核 = 320 核**（无超线程）；`block_dim=24` 时
并发 sim 线程约 **76** 个（72 AICore + 4 AICPU + 主/采集）。要让每个线程**独占一物理核**，绑定的核数必
须 ≥ 线程数：单节点（40 核）不够，至少 2 节点（80 核）；实测 **`--bind node:0,1,2`（120 核 / 3 节点，对
76 线程有约 1.6× 余量、NUMA 跨度最小）** 曲线最干净、单调，即最佳独占档。下面是该策略下的**完整 24 档**
统计（`tasks=480`，7 轮中位数；归档 `build/sweep_1_24_bind120cores.txt`）：
...
|      8 |    24 |                 4.28 |    4.46 |        1.33× |
|      9 |    27 |                 4.03 |    4.19 |        1.25× |
|     10 |    30 |                 4.79 |    4.99 |        1.49× |
|     11 |    33 |                 4.23 |    4.41 |        1.31× |
|     12 |    36 |                 5.27 |    5.49 |        1.64× |
|     13 |    39 |                 5.95 |    6.20 |        1.85× |
|     14 |    42 |                 6.19 |    6.45 |        1.92× |
|     15 |    45 |                 7.16 |    7.46 |        2.22× |
|     16 |    48 |                 6.47 |    6.73 |        2.01× |
|     17 |    51 |                 6.67 |    6.95 |        2.07× |
|     18 |    54 |                 7.52 |    7.83 |        2.33× |
|     19 |    57 |                 8.35 |    8.69 |        2.59× |
|     20 |    60 |                 8.54 |    8.90 |        2.65× |
|     21 |    63 |                 7.77 |    8.10 |        2.41× |
|     22 |    66 |                 9.97 |   10.39 |        3.09× |
|     23 |    69 |                 8.91 |    9.28 |        2.76× |
|     24 |    72 |                 9.27 |    9.65 |        2.87× |

**结论 / 选型。**

* **120 核（3 节点）= 最佳**：每线程独占一核且余量适中，曲线近线性（1→24 约 2.9×、`us/task`
  3.36→9.65），方差远小于不绑核的 §6.2。
* **80 核（`node:0,1`）不够**：对 76 线程是边界，低/中 `block` 干净，但 `block≈24` 超订（线程数 > 核数）
  使 `device` 抖升到约 45 ms（见 `build/sweep_1_24_bind80cores.txt`）——绑核必须**给足核**。
* **160 核（`node:0,1,2,3`）并不更好**：余量更大但跨 4 个 NUMA、与其它租户的重叠面更大，中段反而更
  易抖（见 `build/sweep_1_24_bind160cores.txt`）。
* **共享机注意**：本机为多租户共享，即便绑核，别的任务仍可能占用同一批核；整机繁忙时 `device` 会被
  外部争抢放大（曾观察到全 8 节点 ~100% 占用时数值整体抬升数倍）。干净测量应在空闲窗口进行，可先用
  `mpstat -P ALL 1 1` 确认目标节点空闲。

归档文件名均含绑定核数：不绑核 `build/sweep_1_24.txt`；绑核 `build/sweep_1_24_bind{80,120,160}cores.txt`。
...
**附带优化：`built[]` 后置。** SPMD 下每个核都重放 submit，但一个任务只有 1/3 的核会赢得认领；原先
**所有核**（含注定 type 不匹配/认领失败的 2/3）都先组装了 `built[]`（tc × `sizeof(Tensor)` 拷贝）才
去认领。把 `built[]` 组装移到**认领成功之后**，让失败的核在认领点之前就廉价返回，省掉 2/3 的无用拷贝。
