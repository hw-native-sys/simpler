...
- Remote TASK frames carry each argument's `Tensor` verbatim, and that `Tensor`
  is a `REMOTE_SIDECAR` placeholder: it names the transport alias, carries the
  whole backing `nbytes`, and preserves the view's `byte_offset`; the sidecar
  remains authoritative for the remote locator. Parent virtual addresses and
  the parent's own backings never cross the remote protocol.
