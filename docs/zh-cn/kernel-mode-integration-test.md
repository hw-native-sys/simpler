...
完成。TMR prepare 不同步 stream，因此可以在 capture 内调用；HBG prepare 需在
capture 之前完成专用 AICPU stream 的注册同步。H2D 上传走 `capture_memcpy_h2d`，临时切到 RELAXED 模式后同步拷贝再恢复。
...
- HBG 公开 kernel 调用链已经接通，能力位为 1，并增加了 C ABI 与 Worker 两个 eager 数值用例；本地编译和主机单测已覆盖，真机任务仍需实际获得设备后完成结果确认。
- A5 只经过编译、单元测试与仿真，没有真机结果。
- HBG 数值用例只覆盖一个固定形状的单算子，不覆盖多算子图、其他形状与并发 launch。
- HBG 公开 launch 路径尚未在 ACLGraph capture 窗口内完成真机验证；第 8 节现有公开 capture 场景仍以 TMR 为主。
