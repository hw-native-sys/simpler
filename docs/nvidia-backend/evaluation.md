...
the primary SVG embeds the same graph task-argument metadata for explicit
graph rows, so rendered report artifacts expose the lowered `TaskArgs` shape
instead of leaving it only in raw JSON.
...
visible graph task-argument metadata in `cuda-benchmark.md` and
