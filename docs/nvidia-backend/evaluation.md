...
sweep uses the `6f9a0b78` artifact label:
...
- [Current capture](evaluation-current.md) records a compact tensor-baseline
  shape sweep comparing scalar tensor DAG, WMMA tensor-core DAG, and cuBLAS
  SGEMM rows for `16x16x16` and `16x16x64` descriptors.
...
when the JSON payloads carry that metadata. Persistent smoke rows also include
repeat-run counts and per-launch completion counts, which are the quick audit
fields for graph-descriptor lifecycle reuse captures.
