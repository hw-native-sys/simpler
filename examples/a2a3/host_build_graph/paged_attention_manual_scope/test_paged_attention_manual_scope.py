...
# -----------------------------------------------------------------------------------------------------------
"""Paged attention manual-scope wrapper for A2A3 host_build_graph.

Replaces the automatic same-scope dependency wiring with explicit task-to-task
deps inside PTO2_SCOPE(PTO2ScopeMode::MANUAL). The orchestration source is the
same one the tensormap_and_ringbuffer variant compiles; only the runtime named
by @scene_test differs.

host_build_graph submits the whole graph before the device schedules, so the
ring window must hold every task of a case at once — larger than the
tensormap_and_ringbuffer variant needs, where slots retire as orchestration
proceeds.
"""
...
class TestPagedAttentionManualScopeHostBuildGraph(SceneTestCase):
    RTOL = 1e-3
    ATOL = 1e-3
    ...
    CASES = [
        {
            # 65 792 tasks, all live at once, in a single MANUAL scope.
            "name": "Case1",
            "platforms": ["a2a3"],
