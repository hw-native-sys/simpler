/**
 * AllReduce Orchestration Function
 *
 ...
 * Creates a single AIV task with func_id=0 (allreduce kernel).
 *
 * args layout:
...
int build_allreduce_graph(Runtime* runtime, uint64_t* args, int arg_count) {
    if (arg_count < 5) {
        std::cerr << "build_allreduce_graph: need 5 args, got " << arg_count << '\n';
        return -1;
    }
    ...
    std::cout << "  Created task " << t0 << " (allreduce, func_id=0, AIV)" << '\n';
