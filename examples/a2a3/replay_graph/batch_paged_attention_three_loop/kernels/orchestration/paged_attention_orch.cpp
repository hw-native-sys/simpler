...
__attribute__((visibility("default"))) void aicpu_orchestration_entry(const L2TaskArgs &orch_args) {
                ...
                PTO2_SCOPE() {
                    ...
                    for (uint64_t bn = 0; bn < max_bn; bn++) {
                        PTO2_SCOPE() {
                            ...
                            PTO2TaskId qk_deps[1];
                            if (prev_layer_done.is_valid()) {
                                qk_deps[0] = prev_layer_done;
    ...
    LOG_INFO_V0(
        "batch_paged_attention_three_loop: replay_layers=%" PRIu64 ", task_groups=%" PRIu64 " (batch=%" PRIu64
        ", max_bn=%" PRIu64 ", chunks=%" PRIu64 ", IN_CORE_BATCH=%" PRIu64 ")",
        REPLAY_LAYERS, static_cast<uint64_t>(REPLAY_LAYERS * q_loop * num_chunks * max_bn), batch, max_bn, num_chunks,
        IN_CORE_BATCH
    );
}
