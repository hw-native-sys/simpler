...
# -----------------------------------------------------------------------------------------------------------
"""Qwen3-14B 3-layer decode (load-balanced fused attention) — SceneTestCase.

Self-contained port of pypto-lib ``models/qwen3/14b/decode_layer.py`` entry
``decode_fwd_layers`` with ``_CHUNK_NLAYERS == 3``: a fused chunk of three
...
prompt). Weights + paged KV pool are stacked x3 (one slice per layer; the three
...
from simpler_setup.goldens.qwen3_14b_decode_3layer import (
...
# Validates the fused 3-layer decode against a torch reference.
@scene_test(level=2, runtime="replay_graph")
class TestQwen314BDecode3Layer(SceneTestCase):
    """Three-layer Qwen3-14B decode (decode_fwd_layers N=3) against a torch reference."""
    ...
    CALLABLE = {
        ...
        # kernel_config.py for decode_fwd_layers (N=3). fa_fused is the
