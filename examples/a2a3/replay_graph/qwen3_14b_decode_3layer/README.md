...
# standalone
python examples/a2a3/replay_graph/qwen3_14b_decode_3layer/test_qwen3_14b_decode.py -p a2a3 -d ${DEVICE}
```
...
## Status — single-shot replay

The three layers are submitted into one graph. Orchestration completes and
publishes the frozen dependency graph before scheduler dispatch begins; the
swimlane trace therefore covers one graph build followed by one scheduling
phase, with no per-layer graph boundary or in-replay resource reuse.
