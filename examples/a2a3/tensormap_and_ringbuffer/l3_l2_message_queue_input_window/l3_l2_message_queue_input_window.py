...
# -----------------------------------------------------------------------------------------------------------
"""L3-L2 message queue input-window example."""
...
_ORCH_SRC = os.path.join(_HERE, "kernels", "orchestration", "l3_l2_message_queue_input_window_orch.cpp")
_AIV_SRC = os.path.join(_HERE, "kernels", "aiv", "kernel_input_window_reduce.cpp")
_TIMEOUT_S = 5.0
_QUEUE_DEPTH = 8
...
def _build_chip_callable(platform: str) -> ChipCallable:
    ...
    return ChipCallable.build(
        signature=[],
        func_name="l3_l2_message_queue_input_window_orchestration",
        binary=orch,
...
def run_l3_l2_message_queue_input_window_example(platform: str, device_id: int) -> None:
    ...
    try:
        ...
        def orch(orch_handle, _args, cfg):
            ...
            for payload in _input_payloads():
