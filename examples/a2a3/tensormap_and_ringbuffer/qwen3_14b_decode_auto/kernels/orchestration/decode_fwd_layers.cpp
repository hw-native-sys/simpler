    ...
    SIMPLER_SCOPE() {
        ...
        for (int64_t i = 0; i < 40; i += 1) {
            SIMPLER_SCOPE() {
                ...
                // Private split-K partials: N stacked copies of the accumulator's [16, W] shape,
                // so partial j is rows [j*16, j*16+16). Row stride stays W, which makes a partial's
                // band view geometrically identical to the accumulator band the kernel used to
                // atomically add into — the store keeps its Shape/Stride constants and only drops
                // the atomic.
                uint32_t down_part_all_ci_shapes[2] = {272, 5120};
                TensorCreateInfo down_part_all_ci(down_part_all_ci_shapes, 2, DataType::FLOAT32);
                uint32_t out_part_all_ci_shapes[2] = {80, 5120};
                TensorCreateInfo out_part_all_ci(out_part_all_ci_shapes, 2, DataType::FLOAT32);
                uint32_t gate_part_all_ci_shapes[2] = {80, 17408};
                TensorCreateInfo gate_part_all_ci(gate_part_all_ci_shapes, 2, DataType::FLOAT32);
                uint32_t up_part_all_ci_shapes[2] = {80, 17408};
                TensorCreateInfo up_part_all_ci(up_part_all_ci_shapes, 2, DataType::FLOAT32);
                TaskOutputTensors alloc_2 = alloc_tensors(
                    ...
                    inv_rms_tile_inline126_ci, mlp_tile_inline149_ci, down_part_all_ci, out_part_all_ci,
                    gate_part_all_ci, up_part_all_ci
                );
                ...
                const simpler::tmr::Tensor &down_part_all = alloc_2.get_ref(8);
                const simpler::tmr::Tensor &out_part_all = alloc_2.get_ref(9);
                const simpler::tmr::Tensor &gate_part_all = alloc_2.get_ref(10);
                const simpler::tmr::Tensor &up_part_all = alloc_2.get_ref(11);
                SIMPLER_SCOPE(ScopeMode::AUTO) {
                    ...
                    for (int64_t out_idx_inline74 = 0; out_idx_inline74 < N_OUT_DIRECT_inline61;
                        ...
                        // Each split writes its own [16, 512] partial: rows
                        // [k_split*16, k_split*16+16) of `out_part_all`, column band at
                        // `n_op_inline64`. The splits touch disjoint regions, so TensorMap derives
                        // no edge between them and the kernel stores without an atomic.
                        const simpler::tmr::Tensor out_part_all_band_t12 =
                            out_part_all
                                .slice(
                                    0, static_cast<uint32_t>(k_split_out_inline66 * 16),
                                    static_cast<uint32_t>(k_split_out_inline66 * 16) + 16
                                )
                                .slice(
                                    1, static_cast<uint32_t>(n_op_inline64), static_cast<uint32_t>(n_op_inline64) + 512
                                );
                        params_t12.add_output(out_part_all_band_t12);
                    ...
                    // Task 12r: partials_reduce_hidden_add, one per band the direct split-K tasks
                    // above touched. `out_proj_0`'s SPMD blocks accumulate into the same bands from
                    // a block_idx-derived offset, so the sum joins them atomically. A band holds
                    // only the splits the loop actually emitted — `out_idx` runs to
                    // N_OUT_DIRECT_inline61, so the last band can be short and the unwritten rows
                    // of its slab must stay out of the sum.
                    for (int64_t n_op_reduce = 0; n_op_reduce * 5 < N_OUT_DIRECT_inline61; n_op_reduce += 1) {
                        int64_t splits_here = N_OUT_DIRECT_inline61 - (n_op_reduce * 5);
                        if (splits_here > 5) splits_here = 5;
                        int64_t n_op_r = (n_op_reduce * 512);
                        CoreTaskArgs params_t12r;
                        const simpler::tmr::Tensor out_part_all_band_t12r =
                            out_part_all.slice(0, 0, static_cast<uint32_t>(splits_here * 16))
                                .slice(1, static_cast<uint32_t>(n_op_r), static_cast<uint32_t>(n_op_r) + 512);
                        params_t12r.add_input(out_part_all_band_t12r);
                        const simpler::tmr::Tensor attn_proj_fp32_inline220_band_t12r = attn_proj_fp32_inline220.slice(
                            1, static_cast<uint32_t>(n_op_r), static_cast<uint32_t>(n_op_r) + 512
                        );
                        params_t12r.add_inout(attn_proj_fp32_inline220_band_t12r);
                        params_t12r.add_scalar(splits_here);
                        params_t12r.add_scalar(static_cast<int64_t>(512));
                        TaskOutputTensors task_12r_outs = rt_submit_aiv_task(38, params_t12r);
                        (void)task_12r_outs;
                    }
                    ...
                    // Bands 5..9 are this SPMD form's whole footprint, but its store addresses
                    // them absolutely: block b resolves flat index N_OUT_DIRECT + b to column
                    // (idx / 5) * 512 measured from the buffer base, not from the argument's base.
                    // A narrower view would shift that base and move every store out of range, so
                    // the declaration stays at the parent until the kernel indexes relatively.
                    ...
                    // These five tasks write disjoint 1024-wide bands of `mlp_norm_in` and
                    // `post_norm_partial` with plain stores, one per k_base. Declaring the parents
                    // would order all five against each other even though they never overlap; the
                    // kernel's two stores index relative to these views.
                    const simpler::tmr::Tensor mlp_norm_in_inline71_band_t14 = mlp_norm_in_inline71.slice(
                        1, static_cast<uint32_t>(k_base_inline111), static_cast<uint32_t>(k_base_inline111) + 1024
                    );
                    params_t14.add_inout(mlp_norm_in_inline71_band_t14);
                    const simpler::tmr::Tensor post_norm_partial_inline118_band_t14 = post_norm_partial_inline118.slice(
                        1, static_cast<uint32_t>(k_base_inline111), static_cast<uint32_t>(k_base_inline111) + 1024
                    );
                    params_t14.add_inout(post_norm_partial_inline118_band_t14);
                    ...
                    // would order all five against each other even though they never overlap; the
                    // kernel's two stores index relative to these views.
                    const simpler::tmr::Tensor mlp_norm_in_inline71_band_t15 = mlp_norm_in_inline71.slice(
                        1, static_cast<uint32_t>(k_base_inline111__ssa_v1),
                        static_cast<uint32_t>(k_base_inline111__ssa_v1) + 1024
                    );
                    params_t15.add_inout(mlp_norm_in_inline71_band_t15);
                    const simpler::tmr::Tensor post_norm_partial_inline118_band_t15 = post_norm_partial_inline118.slice(
                        1, static_cast<uint32_t>(k_base_inline111__ssa_v1),
                        static_cast<uint32_t>(k_base_inline111__ssa_v1) + 1024
                    );
                    params_t15.add_inout(post_norm_partial_inline118_band_t15);
                    ...
                    // would order all five against each other even though they never overlap; the
                    // kernel's two stores index relative to these views.
                    const simpler::tmr::Tensor mlp_norm_in_inline71_band_t16 = mlp_norm_in_inline71.slice(
                        1, static_cast<uint32_t>(k_base_inline111__ssa_v2),
                        static_cast<uint32_t>(k_base_inline111__ssa_v2) + 1024
                    );
                    params_t16.add_inout(mlp_norm_in_inline71_band_t16);
                    const simpler::tmr::Tensor post_norm_partial_inline118_band_t16 = post_norm_partial_inline118.slice(
                        1, static_cast<uint32_t>(k_base_inline111__ssa_v2),
                        static_cast<uint32_t>(k_base_inline111__ssa_v2) + 1024
                    );
                    params_t16.add_inout(post_norm_partial_inline118_band_t16);
                    ...
                    // would order all five against each other even though they never overlap; the
                    // kernel's two stores index relative to these views.
                    const simpler::tmr::Tensor mlp_norm_in_inline71_band_t17 = mlp_norm_in_inline71.slice(
                        1, static_cast<uint32_t>(k_base_inline111__ssa_v3),
                        static_cast<uint32_t>(k_base_inline111__ssa_v3) + 1024
                    );
                    params_t17.add_inout(mlp_norm_in_inline71_band_t17);
                    const simpler::tmr::Tensor post_norm_partial_inline118_band_t17 = post_norm_partial_inline118.slice(
                        1, static_cast<uint32_t>(k_base_inline111__ssa_v3),
                        static_cast<uint32_t>(k_base_inline111__ssa_v3) + 1024
                    );
                    params_t17.add_inout(post_norm_partial_inline118_band_t17);
                    ...
                    // would order all five against each other even though they never overlap; the
                    // kernel's two stores index relative to these views.
                    const simpler::tmr::Tensor mlp_norm_in_inline71_band_t18 = mlp_norm_in_inline71.slice(
                        1, static_cast<uint32_t>(k_base_inline111__ssa_v4),
                        static_cast<uint32_t>(k_base_inline111__ssa_v4) + 1024
                    );
                    params_t18.add_inout(mlp_norm_in_inline71_band_t18);
                    const simpler::tmr::Tensor post_norm_partial_inline118_band_t18 = post_norm_partial_inline118.slice(
                        1, static_cast<uint32_t>(k_base_inline111__ssa_v4),
                        static_cast<uint32_t>(k_base_inline111__ssa_v4) + 1024
                    );
                    params_t18.add_inout(post_norm_partial_inline118_band_t18);
                    ...
                    // The SPMD form covers bands 0..5: its 6 blocks each store one 1024-wide
                    // band at `block_idx * 1024`, so [0, 6144) is the whole footprint. Declaring
                    // the parent would order it against every banded write on bands 6..16.
                    const simpler::tmr::Tensor gate_acc_all_inline203_spmd_t20 =
                        gate_acc_all_inline203.slice(1, 0, 6144);
                    params_t20.add_inout(gate_acc_all_inline203_spmd_t20);
                    ...
                    // band at `block_idx * 1024`, so [0, 6144) is the whole footprint. Declaring
                    // the parent would order it against every banded write on bands 6..16.
                    const simpler::tmr::Tensor up_acc_all_inline303_spmd_t21 = up_acc_all_inline303.slice(1, 0, 6144);
                    params_t21.add_inout(up_acc_all_inline303_spmd_t21);
                    ...
                    // band at `block_idx * 1024`, so [0, 6144) is the whole footprint. Declaring
                    // the parent would order it against every banded write on bands 6..16.
                    const simpler::tmr::Tensor gate_acc_all_inline203_spmd_t22 =
                        gate_acc_all_inline203.slice(1, 0, 6144);
                    params_t22.add_inout(gate_acc_all_inline203_spmd_t22);
                    ...
                    // band at `block_idx * 1024`, so [0, 6144) is the whole footprint. Declaring
                    // the parent would order it against every banded write on bands 6..16.
                    const simpler::tmr::Tensor up_acc_all_inline303_spmd_t23 = up_acc_all_inline303.slice(1, 0, 6144);
                    params_t23.add_inout(up_acc_all_inline303_spmd_t23);
                    ...
                    // band at `block_idx * 1024`, so [0, 6144) is the whole footprint. Declaring
                    // the parent would order it against every banded write on bands 6..16.
                    const simpler::tmr::Tensor gate_acc_all_inline203_spmd_t24 =
                        gate_acc_all_inline203.slice(1, 0, 6144);
                    params_t24.add_inout(gate_acc_all_inline203_spmd_t24);
                    ...
                    // band at `block_idx * 1024`, so [0, 6144) is the whole footprint. Declaring
                    // the parent would order it against every banded write on bands 6..16.
                    const simpler::tmr::Tensor up_acc_all_inline303_spmd_t25 = up_acc_all_inline303.slice(1, 0, 6144);
                    params_t25.add_inout(up_acc_all_inline303_spmd_t25);
                    ...
                    // band at `block_idx * 1024`, so [0, 6144) is the whole footprint. Declaring
                    // the parent would order it against every banded write on bands 6..16.
                    const simpler::tmr::Tensor gate_acc_all_inline203_spmd_t26 =
                        gate_acc_all_inline203.slice(1, 0, 6144);
                    params_t26.add_inout(gate_acc_all_inline203_spmd_t26);
                    ...
                    // band at `block_idx * 1024`, so [0, 6144) is the whole footprint. Declaring
                    // the parent would order it against every banded write on bands 6..16.
                    const simpler::tmr::Tensor up_acc_all_inline303_spmd_t27 = up_acc_all_inline303.slice(1, 0, 6144);
                    params_t27.add_inout(up_acc_all_inline303_spmd_t27);
                    ...
                    // band at `block_idx * 1024`, so [0, 6144) is the whole footprint. Declaring
                    // the parent would order it against every banded write on bands 6..16.
                    const simpler::tmr::Tensor gate_acc_all_inline203_spmd_t28 =
                        gate_acc_all_inline203.slice(1, 0, 6144);
                    params_t28.add_inout(gate_acc_all_inline203_spmd_t28);
                    ...
                    // band at `block_idx * 1024`, so [0, 6144) is the whole footprint. Declaring
                    // the parent would order it against every banded write on bands 6..16.
                    const simpler::tmr::Tensor up_acc_all_inline303_spmd_t29 = up_acc_all_inline303.slice(1, 0, 6144);
                    params_t29.add_inout(up_acc_all_inline303_spmd_t29);
                    ...
                    for (int64_t n_out_inline275 = 6; n_out_inline275 < 17; n_out_inline275 += 1) {
                        ...
                        for (int64_t k_split_inline276 = 0; k_split_inline276 < 5; k_split_inline276 += 1) {
                            ...
                            // Each split writes its own [16, 1024] partial: rows
                            // [k_split*16, k_split*16+16) of `gate_part_all`, column band at
                            // `n0_inline122`. The splits touch disjoint regions, so TensorMap derives
                            // no edge between them and the kernel stores without an atomic.
                            const simpler::tmr::Tensor gate_part_all_band_t30 =
                                gate_part_all
                                    .slice(
                                        0, static_cast<uint32_t>(k_split_inline276 * 16),
                                        static_cast<uint32_t>(k_split_inline276 * 16) + 16
                                    )
                                    .slice(
                                        1, static_cast<uint32_t>(n0_inline122),
                                        static_cast<uint32_t>(n0_inline122) + 1024
                                    );
                            params_t30.add_output(gate_part_all_band_t30);
                            ...
                            // Each split writes its own [16, 1024] partial: rows
                            // [k_split*16, k_split*16+16) of `up_part_all`, column band at
                            // `n0_inline122`. The splits touch disjoint regions, so TensorMap derives
                            // no edge between them and the kernel stores without an atomic.
                            const simpler::tmr::Tensor up_part_all_band_t31 =
                                up_part_all
                                    .slice(
                                        0, static_cast<uint32_t>(k_split_inline276 * 16),
                                        static_cast<uint32_t>(k_split_inline276 * 16) + 16
                            ...
                            params_t31.add_output(up_part_all_band_t31);
                        ...
                        // Tasks 30r / 31r: partials_reduce_inter. Bands 6..16 are written only by
                        // the five split-K tasks above, so each band's sum replaces the accumulator
                        // rather than joining it. Bands 0..5 come from the SPMD form and are not
                        // reduced here.
                        CoreTaskArgs params_t30r;
                        const simpler::tmr::Tensor gate_part_all_band_t30r = gate_part_all.slice(
                            1, static_cast<uint32_t>(n0_inline122), static_cast<uint32_t>(n0_inline122) + 1024
                        );
                        params_t30r.add_input(gate_part_all_band_t30r);
                        const simpler::tmr::Tensor gate_acc_all_inline203_band_t30r = gate_acc_all_inline203.slice(
                            1, static_cast<uint32_t>(n0_inline122), static_cast<uint32_t>(n0_inline122) + 1024
                        );
                        params_t30r.add_output(gate_acc_all_inline203_band_t30r);
                        params_t30r.add_scalar(static_cast<int64_t>(5));
                        params_t30r.add_scalar(static_cast<int64_t>(1024));
                        TaskOutputTensors task_30r_outs = rt_submit_aiv_task(39, params_t30r);
                        (void)task_30r_outs;

                        CoreTaskArgs params_t31r;
                        const simpler::tmr::Tensor up_part_all_band_t31r = up_part_all.slice(
                            1, static_cast<uint32_t>(n0_inline122), static_cast<uint32_t>(n0_inline122) + 1024
                        );
                        params_t31r.add_input(up_part_all_band_t31r);
                        const simpler::tmr::Tensor up_acc_all_inline303_band_t31r = up_acc_all_inline303.slice(
                            1, static_cast<uint32_t>(n0_inline122), static_cast<uint32_t>(n0_inline122) + 1024
                        );
                        params_t31r.add_output(up_acc_all_inline303_band_t31r);
                        params_t31r.add_scalar(static_cast<int64_t>(5));
                        params_t31r.add_scalar(static_cast<int64_t>(1024));
                        TaskOutputTensors task_31r_outs = rt_submit_aiv_task(39, params_t31r);
                        (void)task_31r_outs;
                    }
                    for (int64_t n_out_inline292 = 0; n_out_inline292 < 17; n_out_inline292 += 1) {
                        ...
                        // This task writes only the 1024-wide column band at `n0_inline122__ssa_v7`; the
                        // kernel's store is Shape<...,1024> and its base already carries the
                        // view's start_offset. Declaring the whole buffer would make TensorMap
                        // order every band against every other one.
                        const simpler::tmr::Tensor mlp_tile_inline149_band_t32 = mlp_tile_inline149.slice(
                            1, static_cast<uint32_t>(n0_inline122__ssa_v7),
                            static_cast<uint32_t>(n0_inline122__ssa_v7) + 1024
                        );
                        params_t32.add_inout(mlp_tile_inline149_band_t32);
                        // silu reads only the 1024-wide band at `n0` (same offset it writes to
                        // mlp_tile); declaring the whole accumulator would depend on every band.
                        const simpler::tmr::Tensor gate_acc_all_inline203_band_t32 = gate_acc_all_inline203.slice(
                            1, static_cast<uint32_t>(n0_inline122__ssa_v7),
                            static_cast<uint32_t>(n0_inline122__ssa_v7) + 1024
                        );
                        params_t32.add_input(gate_acc_all_inline203_band_t32);
                        // silu reads only the 1024-wide band at `n0` (same offset it writes to
                        // mlp_tile); declaring the whole accumulator would depend on every band.
                        const simpler::tmr::Tensor up_acc_all_inline303_band_t32 = up_acc_all_inline303.slice(
                            1, static_cast<uint32_t>(n0_inline122__ssa_v7),
                            static_cast<uint32_t>(n0_inline122__ssa_v7) + 1024
                        );
                        params_t32.add_input(up_acc_all_inline303_band_t32);
                    ...
                    for (int64_t n_out_inline195 = 0; n_out_inline195 < 5; n_out_inline195 += 1) {
                        ...
                        for (int64_t k_split_inline302 = 0; k_split_inline302 < 17; k_split_inline302 += 1) {
                            ...
                            // down_proj reads only the 1024-wide band at `k0` (its 16 x 64 loads span
                            // k0 .. k0+1024), which is exactly the band one silu task writes.
                            const simpler::tmr::Tensor mlp_tile_inline149_band_t33 = mlp_tile_inline149.slice(
                                1, static_cast<uint32_t>(k0_inline113__ssa_v8),
                                static_cast<uint32_t>(k0_inline113__ssa_v8) + 1024
                            );
                            params_t33.add_input(mlp_tile_inline149_band_t33);
                            params_t33.add_input(ext_w_down);
                            // Each split writes its own [16, 1024] partial: rows
                            // [k_split*16, k_split*16+16) of `down_part_all`, column band at
                            // `n0_inline122__ssa_v8`. The splits touch disjoint regions, so TensorMap
                            // derives no edge between them and the kernel stores without an atomic.
                            const simpler::tmr::Tensor down_part_all_band_t33 =
                                down_part_all
                                    .slice(
                                        0, static_cast<uint32_t>(k_split_inline302 * 16),
                                        static_cast<uint32_t>(k_split_inline302 * 16) + 16
                                    )
                                    .slice(
                                        1, static_cast<uint32_t>(n0_inline122__ssa_v8),
                                        static_cast<uint32_t>(n0_inline122__ssa_v8) + 1024
                                    );
                            params_t33.add_output(down_part_all_band_t33);
                        ...
                        // Task 33r: down_partials_reduce. Sums this band's 17 private partials into
                        // the accumulator. The input view spans all 17 partials' rows, so TensorMap
                        // gives this task 17 unordered producers: the band's readiness chain is one
                        // parallel round plus this task, where the atomic form was 17 deep.
                        CoreTaskArgs params_t33r;
                        const simpler::tmr::Tensor down_part_all_band_t33r = down_part_all.slice(
                            1, static_cast<uint32_t>(n0_inline122__ssa_v8),
                            static_cast<uint32_t>(n0_inline122__ssa_v8) + 1024
                        );
                        params_t33r.add_input(down_part_all_band_t33r);
                        const simpler::tmr::Tensor down_acc_all_inline168_band_t33r = down_acc_all_inline168.slice(
                            1, static_cast<uint32_t>(n0_inline122__ssa_v8),
                            static_cast<uint32_t>(n0_inline122__ssa_v8) + 1024
                        );
                        params_t33r.add_output(down_acc_all_inline168_band_t33r);
                        params_t33r.add_scalar(static_cast<int64_t>(17));
                        params_t33r.add_scalar(static_cast<int64_t>(1024));
                        TaskOutputTensors task_33r_outs = rt_submit_aiv_task(37, params_t33r);
                        (void)task_33r_outs;
                    }
                }
