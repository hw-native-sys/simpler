...
(`src/common/utils/retained_temp_bump.h`) and the packing that sizes it in
`packed_temp_bytes` (`src/common/utils/temp_buffer_plan.h`). Both are one copy
shared by `host_build_graph` and both architectures.

On each trb bind, argument copy-in:

- packs the run's non-child, non-empty tensors to a required size, aligning
  each slice up to 1024 bytes. Kernels require 1024-aligned device pointers and
  `RetainedTempBump` is what makes every slice one: it does not assume the
  backend returns an aligned allocation, because the sim backend's
  `device_malloc` is `std::malloc` and guarantees only `max_align_t`;
- reads the slot via `get_retained_temp_buffer`; if `required + 1023` exceeds
  the retained size it `device_free`s the old buffer, `device_malloc`s a new one
  of that size, and writes it back via `set_retained_temp_buffer` (no data
  preserved across this grow — the buffer is per-run scratch). The extra 1023
  bytes are the headroom the base alignment may consume, so `required` bytes are
  always sliceable whatever address the backend returned; the slot keeps the raw
  pointer, which is what `device_free` must receive. Smaller later runs keep the
...
| `shutdown(thread_idx)` | per thread on exit | Signal EXIT to all of this thread's cores, then wait for each ACK against one shared deadline (`platform_signal_aicore_exit` / `platform_finish_aicore_exit`) |
