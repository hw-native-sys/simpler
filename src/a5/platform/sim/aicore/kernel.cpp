...
static pthread_key_t g_pmu_reg_addrs_key;
...
static void create_tls_keys() {
    ...
    pthread_key_create(&g_pmu_reg_addrs_key, nullptr);
}
...
__aicore__ void set_aicore_pmu_reg_addrs(__gm__ uint64_t *table) {
    pthread_setspecific(g_pmu_reg_addrs_key, reinterpret_cast<void *>(table));
}
__aicore__ uint64_t get_aicore_pmu_reg_base() {
    auto *table = reinterpret_cast<__gm__ uint64_t *>(pthread_getspecific(g_pmu_reg_addrs_key));
    if (table == nullptr) return 0;
    int block_idx = static_cast<int>(reinterpret_cast<uintptr_t>(pthread_getspecific(g_block_idx_key)));
    return table[block_idx];
}
...
extern "C" void aicore_execute_wrapper(
    ...
    // block_idx is required by get_aicore_pmu_reg_base() to index into the
    // PMU reg-addr table; stash it in TLS so the getter can resolve it
    ...
    set_aicore_pmu_reg_addrs(reinterpret_cast<__gm__ uint64_t *>(pmu_reg_addrs));
