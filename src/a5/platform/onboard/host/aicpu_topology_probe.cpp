...
SchedulerClusterAssignment select_scheduler_cluster_assignment(const AicpuTopology &topology) {
    if (is_topology_complete_for_occupy_pool(topology)) {
        return SchedulerClusterAssignment::kContiguous;
    }
    if (topology.source != AicpuTopologySource::kDriver || topology.soc_name.empty() ||
        !topology.device_occupancy.occupy_valid) {
        return SchedulerClusterAssignment::kRoundRobin;
    }

    std::vector<AicpuLogicalCpu> json_cpus;
    if (!load_cpu_topo_from_json(topology.soc_name.c_str(), topology.device_occupancy.occupy, json_cpus)) {
        return SchedulerClusterAssignment::kRoundRobin;
    }
    AicpuTopology json_topology;
    json_topology.source = AicpuTopologySource::kJsonFallback;
    json_topology.device_occupancy = topology.device_occupancy;
    for (const auto &cpu : json_cpus) {
        if (cpu.cpu_id >= 0 && cpu.cpu_id < 64 && ((topology.device_occupancy.occupy >> cpu.cpu_id) & 1ULL) != 0) {
            json_topology.os_schedulable_cpus.push_back(cpu);
        }
    }
    return is_topology_complete_for_occupy_pool(json_topology) ? SchedulerClusterAssignment::kContiguous :
