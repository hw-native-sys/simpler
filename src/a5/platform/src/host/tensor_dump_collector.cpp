 ...
 * buffers + explicit dump_platform_copy_{to,from}_device() hooks,
...
void *DumpMemoryManager::alloc_and_register(size_t size, void **host_ptr_out) {
    ...
    if (register_cb_ != nullptr) {
        ...
        *host_ptr_out = host_ptr;
        ...
        dump_platform_copy_to_device(dev_ptr, host_ptr, size);
        *host_ptr_out = host_ptr;
    }
...
void DumpMemoryManager::process_dump_entry(
    ...
    if (host_ptr != nullptr) {
        dump_platform_copy_from_device(host_ptr, dev_ptr, sizeof(DumpMetaBuffer));
    }
    ...
    dump_platform_copy_from_device(state, dev_state, sizeof(DumpBufferState));
    ...
    while (fq_used < PLATFORM_DUMP_SLOT_COUNT) {
        ...
        dump_platform_copy_to_device(&dev_state->free_queue.tail, &new_tail, sizeof(uint32_t));
...
void DumpMemoryManager::mgmt_loop() {
    ...
    while (running_.load()) {
        ...
        dump_platform_copy_from_device(header->queue_tails, dev_header->queue_tails, sizeof(header->queue_tails));
        ...
        for (int t = 0; t < num_dump_threads_; t++) {
            ...
            while (head != tail) {
                ...
                dump_platform_copy_from_device(&entry, &dev_header->queues[t][head], sizeof(DumpReadyQueueEntry));
                ...
                dump_platform_copy_to_device(&dev_header->queue_heads[t], &head, sizeof(uint32_t));

                did_work = true;
                total_entries_processed++;

                // Re-read tail in case more entries arrived
                dump_platform_copy_from_device(&header->queue_tails[t], &dev_header->queue_tails[t], sizeof(uint32_t));
                rmb();
...
int TensorDumpCollector::initialize(
    ...
    dump_platform_copy_to_device(dump_shared_mem_dev_, dump_shared_mem_host_, shm_size);
...
void TensorDumpCollector::process_dump_buffer(const DumpReadyBufferInfo &info) {
    ...
    if (thread_idx < static_cast<int>(arenas_.size())) {
        ...
        dump_platform_copy_from_device(&state_copy, dev_state, sizeof(DumpBufferState));
        ...
        if (bytes_to_copy > 0) {
            dump_platform_copy_from_device(ai.host_ptr, ai.dev_ptr, bytes_to_copy);
        }
    }
...
int TensorDumpCollector::export_dump_files() {
    ...
    // Clear state for subsequent runs
    collected_.clear();
