...
void *ProfMemoryManager::alloc_and_register(size_t size, void **host_ptr_out) {
    ...
    if (register_cb_ != nullptr) {
        ...
        *host_ptr_out = host_ptr;
        ...
        l2_perf_platform_copy_to_device(dev_ptr, host_ptr, size);
        *host_ptr_out = host_ptr;
    }
...
void ProfMemoryManager::process_ready_entry(
    ...
    if (old_host_ptr != nullptr) {
        ...
        l2_perf_platform_copy_from_device(old_host_ptr, reinterpret_cast<void *>(old_dev_ptr), buf_size);
    }
    ...
    if (is_phase) {
        ...
        l2_perf_platform_copy_from_device(state, dev_state, sizeof(PhaseBufferState));
        ...
        for (int p = 0; p < to_push && available + p < static_cast<uint32_t>(PLATFORM_PROF_SLOT_COUNT); p++) {
            ...
            uint32_t zero = 0;
            l2_perf_platform_copy_to_device(
            ...
            l2_perf_platform_copy_to_device(&dev_state->free_queue.tail, &new_tail, sizeof(uint32_t));
        }
        ...
        l2_perf_platform_copy_from_device(state, dev_state, sizeof(L2PerfBufferState));
...
void ProfMemoryManager::mgmt_loop() {
    ...
    while (running_.load()) {
        ...
        l2_perf_platform_copy_from_device(header->queue_tails, dev_header->queue_tails, sizeof(header->queue_tails));
        ...
        for (int t = 0; t < PLATFORM_MAX_AICPU_THREADS; t++) {
            ...
            while (head != tail) {
                ...
                l2_perf_platform_copy_from_device(&entry, &dev_header->queues[t][head], sizeof(ReadyQueueEntry));
                ...
                l2_perf_platform_copy_to_device(&dev_header->queue_heads[t], &head, sizeof(uint32_t));

                found_any = true;

                // Re-read tail in case more entries arrived
                l2_perf_platform_copy_from_device(
                    &header->queue_tails[t], &dev_header->queue_tails[t], sizeof(uint32_t)
                );
                rmb();
    ...
    for (int t = 0; t < PLATFORM_MAX_AICPU_THREADS; t++) {
        ...
        while (head != tail) {
            ...
            wmb();
            l2_perf_platform_copy_to_device(&dev_header->queue_heads[t], &head, sizeof(uint32_t));
            l2_perf_platform_copy_from_device(&header->queue_tails[t], &dev_header->queue_tails[t], sizeof(uint32_t));
            rmb();
            tail = header->queue_tails[t];
            if (tail >= PLATFORM_PROF_READYQUEUE_SIZE) {
...
int L2PerfCollector::initialize(
    ...
    int rc = l2_perf_platform_copy_to_device(perf_dev_ptr, perf_host_ptr, total_size);
    if (rc != 0) {
    ...
    for (int i = 0; i < num_aicore; i++) {
        ...
        if (host_buf != nullptr) {
            l2_perf_platform_copy_to_device(dev_buf, host_buf, sizeof(L2PerfBuffer));
        }
    }
    for (int t = 0; t < num_phase_threads; t++) {
        PhaseBufferState *state = get_phase_buffer_state(perf_host_ptr, num_aicore, t);
        ...
        void *host_buf = memory_manager_.resolve_host_ptr(dev_buf);
        if (host_buf != nullptr) {
            l2_perf_platform_copy_to_device(dev_buf, host_buf, sizeof(PhaseBuffer));
        }
    }
...
void L2PerfCollector::poll_and_collect(int expected_tasks) {
    ...
    auto read_total_tasks = [&]() -> uint32_t {
        ...
        uint32_t val = 0;
        l2_perf_platform_copy_from_device(&val, &dev_header->total_tasks, sizeof(uint32_t));
...
void L2PerfCollector::scan_remaining_perf_buffers() {
    ...
    for (int i = 0; i < num_aicore_; i++) {
        ...
        l2_perf_platform_copy_from_device(host_state, dev_state, sizeof(L2PerfBufferState));
    }
    rmb();
    ...
    for (int core_index = 0; core_index < num_aicore_; core_index++) {
        ...
        l2_perf_platform_copy_from_device(host_ptr, reinterpret_cast<void *>(buf_ptr), sizeof(L2PerfBuffer));
...
void L2PerfCollector::collect_phase_data() {
    ...
    l2_perf_platform_copy_from_device(host_ph, dev_ph, sizeof(AicpuPhaseHeader));
    for (int t = 0; t < PLATFORM_MAX_AICPU_THREADS; t++) {
        ...
        l2_perf_platform_copy_from_device(host_state, dev_state, sizeof(PhaseBufferState));
    }
    rmb();
    ...
    for (int t = 0; t < total_slots; t++) {
        ...
        if (buf_ptr != 0) {
            ...
            l2_perf_platform_copy_from_device(host_ptr, reinterpret_cast<void *>(buf_ptr), sizeof(PhaseBuffer));
