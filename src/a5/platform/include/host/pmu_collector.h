...
public:
    ...
    /**
     * Device pointer to the per-core PMU MMIO base table
     * (uint64_t[num_cores]). Wire into
     * `KernelArgs::pmu_reg_addrs`. The buffer is allocated by
     * the host but the entries are filled by AICPU `pmu_aicpu_init()`
     * after handshake_all_cores supplies physical_core_ids.
     */
    void *get_pmu_reg_addrs_device_ptr() const { return pmu_reg_addrs_dev_; }
...
private:
    ...
    // Per-core stable PmuAicoreRings + the two per-core address tables that
    // travel through KernelArgs into AICore platform state.
    std::vector<void *> aicore_rings_dev_;
    void *aicore_ring_addrs_dev_ = nullptr;
    void *aicore_ring_addrs_host_ = nullptr;
    void *pmu_reg_addrs_dev_ = nullptr;
    void *pmu_reg_addrs_host_ = nullptr;
