...
extern "C" int comm_global_domain_import(
    ...
    aclError cleanup_status = release_vmm_windows(&allocation->peer_windows);
    if (cleanup_status != ACL_SUCCESS) {
        LOG_ERROR(
            "[global domain rank %d] clear stale peer windows -> %d", allocation->rank, static_cast<int>(cleanup_status)
        );
        return -1;
    }
