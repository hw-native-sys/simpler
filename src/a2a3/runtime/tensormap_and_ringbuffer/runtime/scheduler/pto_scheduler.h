...
struct PTO2SchedulerState {
    ...
    // dispatch_fanin. A consumer whose dispatch_fanin reaches fanin_wait_count (= every
    // DEP_WAIT producer is flagged-and-fully-dispatched, or was already complete when the
    // consumer was wired) is an early-dispatch candidate: CAS NONE->STAGING (exactly-once) and push to
