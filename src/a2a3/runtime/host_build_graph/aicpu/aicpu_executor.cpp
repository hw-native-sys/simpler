...
struct AicpuExecutor {
    ...
    // diagnostic payload: it keeps no shared error scene to preserve.
...
void AicpuExecutor::snapshot_run_terminal(Runtime *runtime) {
    // The shared header outranks a participant's own return: the resolution
    // thread latches a scheduler timeout there and still returns a
    // non-negative completed count, so a record folded from returns alone
    // would call that run a success.
