...
#include "pto_graph_cache.h"     // Graph Execution result and hash helpers
...
// Define or submit a Graph Execution. On a miss the function records the
// sub-DAG; on a hit the function is skipped and Scheduler expands one GRAPH
// task using the current L2TaskArgs snapshot.
...
rt_submit_graph(uint64_t graph_id, PTO2GraphFunction function, const L2TaskArgs &args) {
    ...
    uint64_t structural_key = rt_graph_make_key(graph_id, args);
    PTO2GraphScopeResult result = rt_graph_begin(structural_key, args);
