...
void SchedulerContext::complete_slot_task(
    ...
    if (task_complete && !defer_completion_to_consumer) {
        ...
        completion = sched_->complete_task(slot_state, thread_idx);
#else
        completion = sched_->complete_task(slot_state);
#endif
        consumers_resolved = completion.fanout_edges;
#if SIMPLER_DFX
        if (resolve_t0 != 0) {
...
#endif
        completed_this_turn += completion.stream_tasks_completed;
    }
...
#endif
    if (task_complete && !defer_completion_to_consumer) {
        sched_->retire_graph_node(slot_state);
    }
}
