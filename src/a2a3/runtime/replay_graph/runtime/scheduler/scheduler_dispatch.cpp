...
int32_t SchedulerContext::resolve_and_dispatch(Runtime *runtime, int32_t thread_idx) {
    ...
    // Normally a partial Graph slice immediately returns to the shared queue.
    // Keep one thread-local fallback token so a concurrent host submission
    // cannot consume the just-freed queue cell and make us lose the execution.
    PTO2TaskSlotState *deferred_graph_prepare_slot = nullptr;
    uint64_t deferred_graph_prepare_task_id = 0;
    ...
    while (true) {
        ...
        // Phase 3: Activate dependency-ready Graphs. If preparation has not
        // finished yet, activate_graph_task records the request and the final
        // bounded prepare slice activates the roots later.
        if (thread_idx < 3) {
            constexpr int GRAPH_DRAIN_BATCH = 4;
            PTO2TaskSlotState *graph_batch[GRAPH_DRAIN_BATCH];
            int graph_got = sched_->graph_ready_queue.pop_batch(graph_batch, GRAPH_DRAIN_BATCH);
            for (int gi = 0; gi < graph_got; ++gi) {
                sched_->activate_graph_task(*graph_batch[gi]);
            }
            if (graph_got > 0) made_progress = true;
        }

        // Phase 4: Drain dummy ready queue (S0/S1/S2).
        //
        ...
        if (thread_idx < 3) {
            ...
            for (int di = 0; di < dummy_got; di++) {
                ...
                PTO2SchedulerState::TaskCompletionOutcome completion;
#if SIMPLER_SCHED_PROFILING
                completion = sched_->complete_task(dummy_slot, thread_idx);
#else
                completion = sched_->complete_task(dummy_slot);
#endif
                dummy_consumers = completion.fanout_edges;
#if SIMPLER_DFX
...
#endif
                if (completion.stream_tasks_completed > 0) {
                    int32_t prev =
                        completed_tasks_.fetch_add(completion.stream_tasks_completed, std::memory_order_relaxed);
                    last_progress_count = prev + completion.stream_tasks_completed;
                    cur_thread_completed += completion.stream_tasks_completed;
                }
                sched_->retire_graph_node(dummy_slot);
            }
            if (dummy_got > 0) {
                made_progress = true;
            }
        ...
        // Phase 5: Materialize at most one bounded Graph slice per scheduler
        // iteration. Completion, dummy resolution, normal dispatch, and early
        // dispatch have already run, so a large cached Graph cannot hide
        // AICore finishes behind one monolithic scheduler operation. Requeue a
        // partial execution to round-robin it with other Graph submissions.
        if (thread_idx < 3) {
            uint64_t prepare_task_id = deferred_graph_prepare_task_id;
            PTO2TaskSlotState *prepare_slot = deferred_graph_prepare_slot;
            deferred_graph_prepare_slot = nullptr;
            deferred_graph_prepare_task_id = 0;
            if (prepare_slot == nullptr) {
                prepare_slot = sched_->graph_prepare_queue.pop_tagged(&prepare_task_id);
            }
            if (prepare_slot != nullptr) {
                PTO2TaskDescriptor *prepare_task = prepare_slot->task;
                if (prepare_task != nullptr && prepare_task->kind == PTO2TaskKind::GRAPH &&
                    prepare_task->task_id.raw == prepare_task_id) {
#if SIMPLER_DFX
                    uint64_t graph_prepare_t0 =
                        (l2_swimlane_level_ >= L2SwimlaneLevel::SCHED_PHASES) ? get_sys_cnt_aicpu() : 0;
#endif
                    int32_t nodes_materialized = 0;
                    PTO2GraphMaterializeResult result = sched_->prepare_graph_task(
                        *prepare_slot, PTO2_GRAPH_MATERIALIZE_SLICE_NODES, &nodes_materialized
                    );
                    uint32_t nodes_prepared = static_cast<uint32_t>(nodes_materialized);
                    if (result == PTO2GraphMaterializeResult::PENDING || result == PTO2GraphMaterializeResult::BUSY) {
                        if (!sched_->graph_prepare_queue.push_tagged(prepare_slot, prepare_task_id)) {
                            deferred_graph_prepare_slot = prepare_slot;
                            deferred_graph_prepare_task_id = prepare_task_id;
                        }
                    }
                    if (nodes_prepared > 0) made_progress = true;
#if SIMPLER_DFX
                    if (graph_prepare_t0 != 0 && nodes_prepared > 0) {
                        uint64_t graph_prepare_t1 = get_sys_cnt_aicpu();
                        l2_swimlane_aicpu_record_sched_phase(
                            thread_idx, L2SwimlaneSchedPhaseKind::GraphPrepare, graph_prepare_t0, graph_prepare_t1,
                            sched_l2_swimlane_[thread_idx].sched_loop_count, nodes_prepared
                        );
