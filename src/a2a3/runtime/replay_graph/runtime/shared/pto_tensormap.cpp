 ...
 *
 * Implements TensorMap with a whole-graph entry pool and epoch reset between
 * replays.
 *
 * Key features:
 * 1. O(1) insert at bucket head
 * 2. O(current_graph_entries) lookup
 * 3. No completion-driven retirement or cleanup
 *
...
void PTO2TensorMap::print_stats() {
    ...
    for (int32_t i = 0; i < next_entry_idx; i++)
        if (entry_pool[i].bucket_index != -1) valid++;

    // Count bucket stats
    for (int32_t b = 0; b < num_buckets; b++) {
        int32_t chain_len = 0;
        auto cur_entry = bucket_epochs[b] == current_epoch ? buckets[b] : nullptr;
