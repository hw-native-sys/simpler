...
struct PTO2GraphExecution {
    ...
    // Persistent pool affinity. These describe the immutable node fields that
    // remain valid in node_storage after an execution is retired.
...
PTO2GraphExecution *pto2_graph_execution_create(int32_t node_count, uint64_t graph_key = 0);
...
PTO2GraphMaterializeResult pto2_graph_execution_materialize_slice(
    PTO2TaskSlotState &outer_slot, int32_t max_nodes, int32_t *nodes_materialized = nullptr
);
