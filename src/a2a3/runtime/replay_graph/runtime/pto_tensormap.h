 ...
 * TensorMap structure
 *
 * Hash table backed by a whole-graph bump pool.
 */
struct PTO2TensorMap {
    ...
    // Whole-graph entry storage. The free list is used only for structural
    // replacement/removal while building the graph, never task retirement.
    ...
    /**
     * Phase 1: reserve every sub-region (buckets, entry pool, free list) on the
     * supplied arena. Records the resulting offsets in
     ...
     * for each overlapping entry. Entries remain valid for the entire replay.
     *
     ...
     *
     * Allocates from the whole-graph pool.
