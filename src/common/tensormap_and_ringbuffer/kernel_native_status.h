...
// the onboard transport probe exercises these values on the installed SDK.
...
inline int classify_kernel_dispatch_status(int32_t status, int32_t cleanup_status) noexcept {
    ...
    if (status == static_cast<int>(KernelDispatchStatus::InvalidArgs) ||
        status == static_cast<int>(KernelDispatchStatus::UnsupportedPayload) ||
        status == static_cast<int>(KernelDispatchStatus::InvalidBinding))
        return status;
