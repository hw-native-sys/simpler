...
inline bool make_prepared_kernel_context(
    ...
    if (config.active_callable_id_ != -1 || reinterpret_cast<uint64_t>(config.gm_sm_ptr_) != descriptor.sm_base ||
        reinterpret_cast<uint64_t>(config.prebuilt_arena_base_) != descriptor.arena_base ||
        config.prebuilt_runtime_offset_ != descriptor.runtime_offset || config.orch_args_storage_.tensor_count() != 0 ||
    ...
    const std::array<Region, 5> auxiliary{{
    ...
    const std::array<size_t, auxiliary.size()> alignments{
        alignof(TmrKernelContextDescriptor), alignof(Runtime), alignof(KernelArgs), alignof(TmrLaunchControl),
        alignof(TmrCoreReport)
    };
    ...
    candidate.binding = {
        {descriptor.resident_kernel_args, descriptor.context_generation},
