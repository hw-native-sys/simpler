...
inline int launch_graph_host_args(
    ...
    if (status != ACL_ERROR_RT_MEMORY_ALLOCATION || prelaunch_event == nullptr) return status;
    // AicoreStart is recorded before the current AICPU task and AICore launch.
    // Waiting on AicoreDone or either stream's current tail would form a cycle.
    const int synchronized = aclrtSynchronizeEvent(prelaunch_event);
    return synchronized == 0 ? launch() : synchronized;
}
...
inline int launch_graph_template(
    ...
    aclrtLaunchKernelCfg *config = nullptr, aclrtEvent prelaunch_event = nullptr
) {
    ...
    struct Launch {
        ...
        aclrtEvent prelaunch_event;
    } launch{function, blocks, aicpu_stream, config, prelaunch_event};
    return submit_graph_template(source, {&launch, [](void *ctx, GraphHostArgs &args) {
                                              const auto &call = *static_cast<Launch *>(ctx);
                                              return launch_graph_host_args(
                                                  args, call.function, call.blocks, call.stream, call.config,
                                                  call.prelaunch_event
                                              );
                                          }});
}
