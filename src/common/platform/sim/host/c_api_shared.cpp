 ...
 * to lend. supported() is 0 and init refuses, so no kernel context can exist
 * here and prepare/launch reject with INVALID_STATE. These stubs run the same
 * structural and ordering validation as the real implementation — shared
 * through KernelCtxControlState — and diverge only after every check passes,
 * so an argument a stub accepts is one the onboard path accepts too.
 * =========================================================================== */
