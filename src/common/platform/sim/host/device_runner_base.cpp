...
void SimDeviceRunnerBase::teardown_shared_collectors_after_run(const DfxRunConfig &dfx, uint32_t pipeline_slot) {
    // The host phase records reach the collector before the swimlane export
    // serializes them, and each collector drains before it reconciles before it
    // exports.
