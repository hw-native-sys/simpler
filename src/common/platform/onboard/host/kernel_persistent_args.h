...
/**
 * The complete vocabulary available to argument preparation — the second
 * restricted table alongside KernelContextOps, and the one of the two that may
 * allocate. Synchronization, stream and event
 * operations are absent, so a launch-path operation cannot be written against
 * this table at all.
 *
 * Allocation, release and host-to-device copy arrive as operations rather than
 * a `MemoryAllocator &`, so a host-only test substitutes the whole table and
 * observes the balance of this object's device traffic.
 *
 * `fill_arch_fields` covers the device-side fields that are neither pure
 * memory nor uniform across architectures: the register table, whose arch
 * entry takes its own allocator and a different signature on a2a3 than on a5,
 * and `ffts_base_addr`, which exists only on a2a3.
 */
...
public:
     ...
     * order. A failed rollback release retains its address for finalize_once;
     * preparation stays unavailable until that cleanup succeeds. The original
     * preparation failure code is returned.
     */
    ...
    /**
     * Whether any device block is still owned — including one a failed release
     * retained for a later retry, which is why this is not `is_prepared()`.
     * The aggregate close verdict reads it: a context that released its streams
     * and events but still owns an argument block is not closed.
     */
    bool has_live_resources() const {
        return device_k_args_ != nullptr || args_.runtime_args != nullptr || args_.regs != 0;
    }
     ...
     * the first failure code is returned.
     */
    int finalize_once();
