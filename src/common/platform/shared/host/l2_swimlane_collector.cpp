...
int L2SwimlaneCollector::initialize(
    ...
    // register_cb may legitimately be null (non-SVM platforms have no halHostRegister); alloc
    ...
    // manager-tracked device buffer + non-SVM host shadow allocated so far via the
    ...
    // via the base allocator. On non-SVM platforms there is no halHostRegister and the
