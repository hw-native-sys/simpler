...
typedef struct {
    ...
    size_t dep_list_pool_size;            // Dependency list pool size
    
    // Offsets into shared memory (relative to SM_Base)
    size_t task_descriptors_offset;       // Offset to TaskDescriptor array
    size_t dep_list_pool_offset;          // Offset to DepListPool
    ...
    volatile size_t graph_output_size;    // Size in bytes
...
typedef struct {
    ...
    size_t  sm_size;              // Total size of shared memory
 ...
 */
size_t pto2_sm_calculate_size(size_t task_window_size, size_t dep_list_pool_size);
 ...
 */
PTO2SharedMemoryHandle* pto2_sm_create(size_t task_window_size,
                                        size_t heap_size,
                                        size_t dep_list_pool_size);
...
static inline PTO2TaskDescriptor* pto2_sm_get_task(PTO2SharedMemoryHandle* handle,
                                                    int32_t task_id) {
    size_t window_mask = handle->header->task_window_size - 1;
