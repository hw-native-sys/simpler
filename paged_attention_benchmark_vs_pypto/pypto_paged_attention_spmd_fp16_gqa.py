    ...
    class SpmdPagedAttentionProgram:
            ...
            for b in pl.range(spmd_idx, batch_count, spmd_block_num):
                ...
                if valid_len > 0:
                    ...
                    qi_l1 = pl.load(
                        ...
                        valid_shapes=[active_q_tile, head_dim],
                    )
                    kj_l1 = pl.load(
                        key_cache,
                        [kj_row, 0],
                    ...
                    sij_batch = pl.store(sij_l0c, [b * q_tile, 0], sij_batch, [active_q_tile, block_size])
            return sij_batch
        ...
        def SpmdSoftmaxPrepare(
            ...
            for b in pl.range(spmd_idx, batch_count, spmd_block_num):
                ...
                if valid_len == 0:
                    ...
                    zero_pij_tile = pl.cast(zero_pij_f32_tile, target_type=pl.FP16)
                    ...
                    pij_batch = pl.store(zero_pij_tile, [b * q_tile, 0], pij_batch, [active_q_tile, block_size])
                    mij_batch = pl.store(neg_inf_mi_tile, [b * q_tile, 0], mij_batch, [active_q_tile, 1])
                    lij_batch = pl.store(zero_li_tile, [b * q_tile, 0], lij_batch, [active_q_tile, 1])
                else:
                    s_tile = pl.load(
                        sij_batch,
                        [b * q_tile, 0],
                        [q_tile, block_size],
                        target_memory=pl.MemorySpace.Vec,
                        valid_shapes=[active_q_tile, valid_len],
                    )
                    ...
                    pij_batch = pl.store(pij_tile_f16, [b * q_tile, 0], pij_batch, [active_q_tile, block_size])
                    mij_batch = pl.store(mi_tile, [b * q_tile, 0], mij_batch, [active_q_tile, 1])
                    lij_batch = pl.store(li_tile, [b * q_tile, 0], lij_batch, [active_q_tile, 1])
            ...
            for b in pl.range(spmd_idx, batch_count, spmd_block_num):
                ...
                if valid_len > 0:
                    ...
                    pij_l1 = pl.load(
                        ...
                        [q_tile, block_size],
                        target_memory=pl.MemorySpace.Mat,
                        valid_shapes=[active_q_tile, block_size],
                    )
                    vj_l1 = pl.load(
                        value_cache,
                        [vj_row, 0],
                    ...
                    oi_new_batch = pl.store(oi_l0c, [b * q_tile, 0], oi_new_batch, [active_q_tile, head_dim])
            return oi_new_batch
        ...
        def SpmdOnlineUpdate(  # noqa: PLR0913
            ...
            if is_first != 0:
                ...
                for b in pl.range(spmd_idx, batch_count, spmd_block_num):
                    ...
                    mi_batch = pl.store(mi_new_dn, [b * q_tile, 0], mi_batch, [active_q_tile, 1])
                    li_batch = pl.store(li_updated_dn, [b * q_tile, 0], li_batch, [active_q_tile, 1])
                    ...
                    oi_batch = pl.store(oi_updated, [b * q_tile, 0], oi_batch, [active_q_tile, head_dim])
        ...
        def SpmdNormalize(
            ...
            for b in pl.range(spmd_idx, batch_count, spmd_block_num):
                ...
                out_tensor = pl.store(dst_tile, [dst_row, 0], out_tensor, [active_q_tile, head_dim])
            return out_tensor
        ...
        def paged_attention_spmd(
            ...
            for q_idx in pl.range(q_loop_cfg):
                ...
                for bn in pl.range(max_bn_cfg):
                    ...
                    pij_b = pl.create_tensor([batch_q_tile, block_size], dtype=pl.FP16)
...
def golden(tensors: dict, params: dict | None = None) -> None:
    ...
    num_kv_heads = int(config[2].item())
    ...
    for kv_head_idx in range(num_kv_heads):
        ...
        for bn in range(max_bn):
            ...
            kj_all = key_cache[block_indices, kv_head_idx]
            vj_all = value_cache[block_indices, kv_head_idx]
...
def build_tensor_specs(
    ...
    if context_lens_init.numel() != batch:
        raise ValueError(f"context_len must provide exactly {batch} elements, got {context_lens_init.numel()}")
    ...
    return [
        TensorSpec("query", [query_rows, head_dim], torch.float16, init_value=torch.randn),
        TensorSpec("key_cache", [key_cache_rows, head_dim], torch.float16, init_value=torch.randn),
        TensorSpec("value_cache", [key_cache_rows, head_dim], torch.float16, init_value=torch.randn),
