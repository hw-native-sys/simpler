...
class Worker:
    ...
    def _resolved_global_nodes(self) -> dict[int, _GlobalNodeRuntime]:
        ...
        if len(self._next_level_worker_ids) != len(self._next_level_workers):
            raise ValueError("next-level worker ids/workers length mismatch")
    ...
    def _drain_pending_global_domain_snapshot(
        self, resources: _RunResources, pending: list[GlobalCommDomainHandle]
    ) -> None:
        """Free a snapshot while each source-queue claim stays durable."""
        ...
        try:
            ...
            cursor = _RunFinalizationCursor(
                steps=(
                    ...
                    ("pending_global_domains", lambda: self._execute_pending_global_domain_releases(resources)),
                    (
                        "live_global_domains",
                        lambda: self._release_all_live_global_domains(resources, include_retained=False),
                    ),
    ...
    def _teardown_worker_tree(  # noqa: PLR0912 -- one ordered driver for rollback and close resource classes
        ...
        # entries. Any survivor fences the transport teardown below. Global
        # CommDomains release before local CommDomains: once `dw.close()` runs
        # the chip mailboxes are unusable and neither can drive its release
        # controls.
