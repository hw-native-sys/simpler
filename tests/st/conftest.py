...
def drain_host_log(tmp_path_factory):
    ...
    logger to a pytest session directory before runs can bind it to a capture
    directory that the test deletes between runs. A session directory also
    survives per-test cleanup when tmp_path_retention_policy is failed.
