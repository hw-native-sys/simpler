...
def _verify_runtime_clock_logs(monkeypatch, st_platform):
    ...
    def finalize(case_label, output_prefix, **kwargs):
        ...
        return original(case_label, output_prefix, **kwargs)
...
class TestConsecutiveClockAlignment(SceneTestCase):
    ...
    def test_run(self, st_platform, st_worker, request, monkeypatch):
        _verify_runtime_clock_logs(monkeypatch, st_platform)
        outputs_dir = _outputs_dir()
        previous_outputs = set(outputs_dir.iterdir()) if outputs_dir.exists() else set()
        super().test_run(st_platform, st_worker, request)
        level = self._effective_enable_chip_swimlane(request)
        if level < 3 or (level == 3 and not os.environ.get("SIMPLER_HBG_HOST_PHASE_RECORDS_ENABLE")):
            return

        invocations = []
        for case in self._matching_cases(st_platform, request):
            label = _sanitize_for_filename(f"TestConsecutiveClockAlignment_{case['name']}")
            matches = [path for path in outputs_dir.glob(f"{label}_*") if path not in previous_outputs]
            assert len(matches) == 1, f"expected one new output for {label}: {matches}"
            captures = list(matches[0].rglob("chip_swimlane_records.json"))
            assert len(captures) == 1
            capture = captures[0]
            raw = json.loads(capture.read_text())
            merged = json.loads((matches[0] / "l3_swimlane.json").read_text())
            ...
            assert rank_metadata[0]["input"] == str(capture)
            ...
            assert all(
                span.name in {"chip.run", "chip.run.runner_run", "chip.run.runner_run.aicpu_launch"}
                or span.name == "chip.run.runner_run.device_wall"
                or span.name.startswith("chip.run.runner_run.device_wall.")
                for span in spans
            )
            ...
            assert {
                ...
                "chip.run.runner_run.aicpu_launch",
            } <= names
