 ...
 *
 * Memory Layout:
 *   Query: (batch, seq_len, num_heads, head_dim) bf16
...
__attribute__((visibility("default"))) void aicpu_orchestration_entry(TaskArg* orch_args, int orch_thread_num, int orch_thread_index) {
    ...
    uint64_t seq_len   = orch_args[0].tensor.shapes[1];
    uint64_t num_heads = orch_args[0].tensor.shapes[2];
    uint64_t head_dim  = orch_args[0].tensor.shapes[3];
    ...
    uint64_t kv_head_num  = orch_args[1].tensor.shapes[2];
    ...
    // 4D external tensors matching input data shapes
    uint32_t query_shapes[4] = {(uint32_t)batch, (uint32_t)seq_len, (uint32_t)num_heads, (uint32_t)head_dim};
    uint32_t key_cache_shapes[4] = {(uint32_t)total_blocks_count, (uint32_t)block_size, (uint32_t)kv_head_num, (uint32_t)head_dim};
    ...
    Tensor key_cache = make_tensor_external(kc_ptr, key_cache_shapes, 4, data_type, false);
    Tensor value_cache = make_tensor_external(vc_ptr, value_cache_shapes, 4, data_type, false);
    Tensor out = make_tensor_external(out_ptr, out_shapes, 4, DataType::FLOAT32);
    ...
    for (uint64_t b_idx = 0; b_idx < batch; b_idx++) {
        ...
        for (uint64_t q_idx = 0; q_idx < q_loop; q_idx++) {
            ...
            PTO2_SCOPE() {
                ...
                uint32_t qi_shapes[4] = {1, 1, (uint32_t)q_tile, (uint32_t)head_dim};
                uint32_t qi_offsets[4] = {(uint32_t)b_idx, 0, (uint32_t)(q_idx * q_tile), 0};
                Tensor qi = query.view(qi_shapes, qi_offsets);
                uint32_t out_view_shapes[4] = {1, 1, (uint32_t)q_tile, (uint32_t)head_dim};
                uint32_t out_view_offsets[4] = {(uint32_t)b_idx, 0, (uint32_t)(q_idx * q_tile), 0};
