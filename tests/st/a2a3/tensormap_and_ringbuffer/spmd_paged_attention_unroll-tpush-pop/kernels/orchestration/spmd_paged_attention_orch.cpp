 ...
 *
 * Submits a single MixedKernels task that processes ALL KV blocks per
 * (batch, q_tile) position. AIC and AIV cooperate via TPUSH/TPOP pipes:
 *   AIC: QK matmul → TPUSH(sij) → TPOP(pij) → PV matmul → TPUSH(oi_new)
 *   AIV: TPOP(sij) → online softmax → TPUSH(pij) → TPOP(oi_new) → online update
 *
 * SPMD parallelism: block_num = batch * q_loop.
 * Each SPMD block handles one (batch_idx, q_tile_idx).
 *
 * GM FIFO buffers for TPUSH/TPOP are allocated as scratch tensors.
 * Each SPMD block gets its own FIFO slots.
 */
...
// GM FIFO slot sizes (must match kernel's constants)
static constexpr uint32_t SIJ_SLOT_SIZE = Q_TILE * 128 * sizeof(float);      // 8192
static constexpr uint32_t PIJ_SLOT_SIZE = Q_TILE * 128 * sizeof(uint16_t);   // 4096 (bf16)
static constexpr uint32_t OI_SLOT_SIZE = Q_TILE * HEAD_DIM * sizeof(float);  // 8192
static constexpr uint32_t FIFO_DEPTH = 1;
...
__attribute__((visibility("default"))) void aicpu_orchestration_entry(const ChipStorageTaskArgs &orch_args) {
    ...
    LOG_INFO(
        "SPMD PA TPUSH/TPOP: batch=%" PRIu64 " heads=%" PRIu64 " hd=%" PRIu64 " bs=%" PRIu64 " q_loop=%" PRIu64
        " blocks=%d",
    ...
    // GM FIFO buffers for TPUSH/TPOP (one set of slots per SPMD block)
    ...
    LOG_INFO("SPMD PA TPUSH/TPOP: submitted 1 MixedKernels task, blocks=%d", static_cast<int>(spmd_block_num));
}
