 ...
 */
/**
 * SPMD Paged Attention Orchestration with N_UNROLL block batching
 *
 ...
 *
 * Batches up to N_UNROLL KV blocks per group. Each group submits 4 tasks:
 *   1. QK matmul:  qi @ K^T for n_blocks → sij (Q_TILE, n_blocks * block_size)
 *   2. Softmax:    two-pass over sij → pij, mij, lij
 ...
 *
 * Softmax and online-update run as MIX tasks (AIC hub + AIV0 + AIV1) with
 * dual-vector subvector partitioning via get_sub_block_id().
 *
 * Memory Layout:
...
__attribute__((visibility("default"))) void aicpu_orchestration_entry(const ChipStorageTaskArgs &orch_args) {
    ...
    LOG_INFO(
        "SPMD PA Unroll: batch=%" PRIu64 " heads=%" PRIu64 " hd=%" PRIu64 " bs=%" PRIu64 " q_loop=%" PRIu64
        " blocks=%d N_UNROLL=%d",
        batch, num_heads, head_dim, block_size, q_loop, spmd_block_num, N_UNROLL
    );
    ...
    // Accumulator create infos (persistent across the bn loop)
    uint32_t n_rows = static_cast<uint32_t>(spmd_block_num) * static_cast<uint32_t>(Q_TILE);
    uint32_t acc_oi_shapes[2] = {n_rows, static_cast<uint32_t>(head_dim)};
    uint32_t scalar_shapes[1] = {n_rows};
    TensorCreateInfo acc_oi_ci(acc_oi_shapes, 2, DataType::FLOAT32);
    ...
    PTO2_SCOPE() {
        ...
        for (uint64_t bn = 0; bn < max_bn; bn += N_UNROLL) {
            ...
            // Scratch tensor create infos sized for n_blocks per SPMD block
            uint32_t sij_shapes[2] = {n_rows, static_cast<uint32_t>(n_blocks * block_size)};
            uint32_t pij_shapes[2] = {n_rows, static_cast<uint32_t>(n_blocks * block_size)};
            uint32_t oi_new_shapes[2] = {n_rows, static_cast<uint32_t>(head_dim)};
            uint32_t mij_shapes[1] = {n_rows};
            ...
            TensorCreateInfo mij_ci(mij_shapes, 1, DataType::FLOAT32);
            TensorCreateInfo lij_ci(lij_shapes, 1, DataType::FLOAT32);

            // -- QK Matmul (AIC, SPMD): n_blocks matmuls per SPMD block --
            Arg qk_args;
            ...
            qk_args.add_scalar(static_cast<int64_t>(n_blocks));
            ...
            // Two-pass softmax over n_blocks tiles per SPMD block
            Arg sf_args;
            ...
            sf_args.add_scalar(static_cast<int64_t>(n_blocks));
            ...
            // -- PV Matmul (AIC, SPMD): SplitK accumulated across n_blocks --
            Arg pv_args;
            ...
            pv_args.add_scalar(static_cast<int64_t>(n_blocks));
    ...
    uint64_t n_groups = (max_bn + N_UNROLL - 1) / N_UNROLL;
    LOG_INFO(
        "SPMD PA Unroll: %" PRIu64 " groups x 4 tasks, blocks=%d", n_groups, static_cast<int>(spmd_block_num)
    );
}
