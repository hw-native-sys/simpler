...
// A wire payload carries addresses, not pointers.
...
void sequence(
    ...
    core_launch(binary, caller, {addr(p[0]), 0, addr(p[1]), 3});
    ...
    core_launch(binary, core, {addr(p[1]), 0, addr(p[3]), 11});
    record(state, KernelEventKind::AicoreDone, core);
    CaptureArgs cpu_args{addr(p[1]), 0, addr(p[2]), 7};
    ...
    core_launch(binary, caller, {addr(p[2]), addr(p[3]), addr(p[4]), 0});
}
}  // namespace
