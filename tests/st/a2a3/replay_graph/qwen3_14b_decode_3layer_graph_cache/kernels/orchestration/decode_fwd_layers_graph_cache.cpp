...
__attribute__((visibility("default"))) void aicpu_orchestration_entry(const L2TaskArgs &orch_args) {
    ...
    PTO2_SCOPE() {
        ...
        for (int64_t i = 0; i < 3; i += 1) {
            ...
            L2TaskArgs layer_args;
            layer_args.add_input(cur__rv_v7);
            layer_args.add_output(next_hidden);
            layer_args.add_input(ext_input_rms_weight);
            layer_args.add_input(ext_wq);
            layer_args.add_input(ext_wk);
            layer_args.add_input(ext_wv);
            layer_args.add_input(q_norm_w_inline242);
            layer_args.add_input(k_norm_w_inline138);
            layer_args.add_input(ext_seq_lens);
            layer_args.add_input(ext_block_table);
            layer_args.add_input(ext_slot_mapping);
            layer_args.add_input(ext_rope_cos);
            layer_args.add_input(ext_rope_sin);
            layer_args.add_inout(ext_k_cache);
            layer_args.add_inout(ext_v_cache);
            layer_args.add_input(ext_wo);
            layer_args.add_input(ext_w_gate);
            layer_args.add_input(ext_w_up);
            layer_args.add_input(ext_w_down);
            layer_args.add_input(ext_post_rms_weight);
            layer_args.add_scalar(
                i, layer_hidden_base_inline135, layer_inter_base_inline231, layer_cache_base_inline137
            );
            auto layer_definition = [](const L2TaskArgs &args) {
                const Tensor &cur__rv_v7 = args.tensor(0).ref();
                const Tensor &next_hidden = args.tensor(1).ref();
                const Tensor &ext_input_rms_weight = args.tensor(2).ref();
                const Tensor &ext_wq = args.tensor(3).ref();
                const Tensor &ext_wk = args.tensor(4).ref();
                const Tensor &ext_wv = args.tensor(5).ref();
                const Tensor &q_norm_w_inline242 = args.tensor(6).ref();
                const Tensor &k_norm_w_inline138 = args.tensor(7).ref();
                const Tensor &ext_seq_lens = args.tensor(8).ref();
                const Tensor &ext_block_table = args.tensor(9).ref();
                const Tensor &ext_slot_mapping = args.tensor(10).ref();
                const Tensor &ext_rope_cos = args.tensor(11).ref();
                const Tensor &ext_rope_sin = args.tensor(12).ref();
                const Tensor &ext_k_cache = args.tensor(13).ref();
                const Tensor &ext_v_cache = args.tensor(14).ref();
                const Tensor &ext_wo = args.tensor(15).ref();
                const Tensor &ext_w_gate = args.tensor(16).ref();
                const Tensor &ext_w_up = args.tensor(17).ref();
                const Tensor &ext_w_down = args.tensor(18).ref();
                const Tensor &ext_post_rms_weight = args.tensor(19).ref();
                auto i = args.scalar(0);
                auto layer_hidden_base_inline135 = args.scalar(1);
                auto layer_inter_base_inline231 = args.scalar(2);
                auto layer_cache_base_inline137 = args.scalar(3);
                PTO2_SCOPE() {
                    uint32_t normed_states_inline146_ci_shapes[2] = {16, 5120};
                    TensorCreateInfo normed_states_inline146_ci(
                        normed_states_inline146_ci_shapes, 2, DataType::BFLOAT16
                    );
                    ...
                    TensorCreateInfo inv_rms_states_inline160_ci(
                        inv_rms_states_inline160_ci_shapes, 2, DataType::FLOAT32
                    );
                    ...
                    TaskOutputTensors alloc_1 = alloc_tensors(
                        ...
                        attn_out_inline157_ci, all_oi_tmp_inline145_ci, all_cur_mi_inline174_ci,
                        all_cur_li_inline190_ci, fa_work_table_inline126_ci, fa_total_inline183_ci,
                        q_proj_norm_inline197_ci, k_proj_norm_inline155_ci
                    );
                    ...
                    TensorCreateInfo attn_proj_fp32_inline248_ci(
                        attn_proj_fp32_inline248_ci_shapes, 2, DataType::FLOAT32
                    );
                    ...
                    uint32_t inv_rms_col_inline89_shapes[2] = {
                        (inv_rms_col_inline89_offsets[0] >= inv_rms_states_inline160.shapes[0] ?
                             0u :
                             std::min<uint32_t>(
                                 16, inv_rms_states_inline160.shapes[0] - inv_rms_col_inline89_offsets[0]
                             )),
                        (inv_rms_col_inline89_offsets[1] >= inv_rms_states_inline160.shapes[1] ?
                             0u :
                             std::min<uint32_t>(
                                 1, inv_rms_states_inline160.shapes[1] - inv_rms_col_inline89_offsets[1]
                             ))
                    };
                    ...
                    PTO2_SCOPE(PTO2ScopeMode::MANUAL) {
                        ...
                        for (int64_t n_out_proj_inline71 = 0; n_out_proj_inline71 < 10; n_out_proj_inline71 += 1) {
                            ...
                            for (int64_t k_split_out_inline193 = 0; k_split_out_inline193 < 5;
                                 k_split_out_inline193 += 1) {
                                ...
                                out_tids_inline268[((n_out_proj_inline71 * 5) + k_split_out_inline193)] =
                                    out_tid_inline269;
                            }
                        }
            ...
            rt_submit_graph(PTO2_GRAPH_KEY("qwen3_decode_3layer_layer_v1"), +layer_definition, layer_args);
            if (i == 2) {
