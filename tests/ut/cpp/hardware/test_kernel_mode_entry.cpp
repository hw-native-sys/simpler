 ...
 *   three                             -> PTO_RUNTIME_ERR_INVALID_ARGUMENT, and the
 *                                        refusal is reached before the state
 *                                        check, so validation is what rejects
 *
 * Distinguishing INVALID_ARGUMENT from INVALID_STATE is what makes the pass meaningful:
...
TEST_F(KernelModeEntryTest, EntriesRefuseOnBorrowedStreamPerContract) {
    ...
    int32_t callable_id = 7;
    EXPECT_EQ(
        api.prepare_callable(ctx, image.data(), AlignedCallableImage::size(), &callable_id),
        PTO_RUNTIME_ERR_INVALID_STATE
    ) << "no kernel context is live on this device context";
    EXPECT_EQ(callable_id, -1) << "a failed prepare reports -1 through out_callable_id";
...
TEST_F(KernelModeEntryTest, MalformedArgumentsAreRejectedBeforeTheStateCheck) {
    ...
    int32_t callable_id = 7;
    EXPECT_EQ(
        api.prepare_callable(ctx, nullptr, AlignedCallableImage::size(), &callable_id), PTO_RUNTIME_ERR_INVALID_ARGUMENT
    );
    EXPECT_EQ(callable_id, -1);
    // Below the size floor the image cannot hold a ChipCallable header.
    callable_id = 7;
    EXPECT_EQ(
        api.prepare_callable(ctx, image.data(), sizeof(ChipCallable) - 1, &callable_id),
        PTO_RUNTIME_ERR_INVALID_ARGUMENT
    );
    EXPECT_EQ(callable_id, -1);
    // Without somewhere to write the ID the call cannot report one.
    EXPECT_EQ(
...
TEST_F(KernelModeEntryTest, WorkerKernelInitReachesTheEntryAndReportsUnsupported) {
    ...
    try {
        ...
        ADD_FAILURE() << "kernel_init must reach simpler_kernel_mode_init and surface its refusal";
    } catch (const UnsupportedRuntimeOperation &error) {
        EXPECT_EQ(error.code(), PTO_RUNTIME_ERR_UNSUPPORTED);
    }
