...
TEST_F(HbgGraphSubmitFailureTest, ExplicitDependencyRejectsNonzeroRingId) {
    ...
    const PTO2TaskId invalid_dep = PTO2TaskId::make(1, producer.task_id().local());
    ...
    EXPECT_EQ(
        ...
        static_cast<int32_t>(PTO2_ERROR_INVALID_ARGS)
    );
    const PTO2TaskSlotState &producer_slot =
...
TEST_F(HbgGraphSubmitFailureTest, CachedGraphUsesFinalTaskCapacitySlot) {
    std::array<uint32_t, 16> storage{};
    uint32_t shape[] = {static_cast<uint32_t>(storage.size())};
    ChipTensor boundary = make_tensor_external(storage.data(), shape, 1);
    ...
    while (allocator.active_count() < allocator.task_capacity() - 1) {
    ...
    EXPECT_EQ(replay.task_id.local(), static_cast<uint32_t>(allocator.task_capacity() - 1));
    EXPECT_EQ(allocator.active_count(), allocator.task_capacity());
    EXPECT_EQ(sm_handle->header->ring.fc.current_task_index.load(std::memory_order_acquire), allocator.task_capacity());
