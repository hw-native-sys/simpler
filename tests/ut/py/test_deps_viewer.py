...
def emit_text(*args, **kwargs):
    kwargs.setdefault("runtime_name", TMR)
...
def test_main_passes_task_table_when_show_tensor_info_enabled(tmp_path, monkeypatch):
    ...
    deps_json.write_text(json.dumps({"runtime": TMR}))
...
def _write_deps_edges(tmp_path, edges, sources=None, runtime: Optional[str] = TMR):
...
def test_deps_runtime_reads_the_name_from_the_document(tmp_path):
    (tmp_path / "a").mkdir()
    named = _write_deps_edges(tmp_path / "a", [(1, 2)], runtime=HBG)

    assert deps_viewer._deps_runtime(named) == HBG
