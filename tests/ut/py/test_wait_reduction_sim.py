...
from simpler_setup.tools.swimlane_converter import HBG_RUNTIME as HBG
from simpler_setup.tools.swimlane_converter import TMR_RUNTIME as TMR
...
def test_scope_key_reads_each_runtimes_own_boundary():
    ...
    assert _scope_key((1 << 32) | 7, TMR) == _scope_key((1 << 32) | 9, TMR)
    assert _scope_key((1 << 32), TMR) != _scope_key((2 << 32), TMR)

    # hbg: one modular task's body is one scope, whatever the index within it.
    assert _scope_key(_hbg_sub_task(3, 0), HBG) == _scope_key(_hbg_sub_task(3, 9), HBG)
    assert _scope_key(_hbg_sub_task(3, 0), HBG) != _scope_key(_hbg_sub_task(4, 0), HBG)
    # Tasks of the run itself share one scope, and it is not any body's.
    assert _scope_key(1, HBG) == _scope_key(2, HBG)
    assert _scope_key(1, HBG) != _scope_key(_hbg_sub_task(0, 1), HBG)
