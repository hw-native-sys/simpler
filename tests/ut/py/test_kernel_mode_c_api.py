...
def test_kernel_lifecycle_retry(arch, runtime, scenario, kernel_close_faults, request):
    ...
    if scenario == "fatal_device":
        # The quarantine is observable: finalize says it retained the
        # resources, and destruction is refused because they are still owned.
        combined = result.stdout + result.stderr
        assert "quarantined — resources retained for an explicit close" in combined
